...
namespace FortitudeMarketsCore.Pricing.PQ.LastTraded.LastTradeEntrySelector
{
    ...
    {
        protected override IPQRecentlyTradedFactory SelectSimpleLastTradeEntry(IPQSourceTickerQuoteInfo sourceTickerQuoteInfo)
        {
            return new PQLastTradeFactory();
        }

        protected override IPQRecentlyTradedFactory SelectLastPaidGivenTradeEntry(IPQSourceTickerQuoteInfo sourceTickerQuoteInfo)
        {
            return new PQLastPaidGivenTradeFactory();
        }

        protected override IPQRecentlyTradedFactory SelectTraderLastTradeEntry(IPQSourceTickerQuoteInfo sourceTickerQuoteInfo)
        {
        ...
        {
            if (copySourceType == typeof(PQLastTrade) || copySourceType == typeof(LastTrade))
            {
                return true;
            }
