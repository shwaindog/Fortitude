...
namespace FortitudeMarketsCore.Pricing.PQ.LayeredBook
{
    public class PQSourcePriceVolumeLayer : PQPriceVolumeLayer, IPQSourcePriceVolumeLayer
    {
        ...
        public override void CopyFrom(IPriceVolumeLayer source, CopyMergeFlags copyMergeFlags = CopyMergeFlags.Default)
        {
            ...
            IPQSourcePriceVolumeLayer pqspvl = source as IPQSourcePriceVolumeLayer;
            ...
            else if (pqspvl != null)
            {
                ...
                if (pqspvl.IsSourceNameUpdated)
                {
                    SourceId = pqspvl.SourceId;
                }
                if (pqspvl.IsExecutableUpdated)
                {
        ...
        {
            return new PQSourcePriceVolumeLayer(this);
        }
        ...
        {
            return $"PQSourcePriceVolumeLayer {{ {nameof(Price)}: {Price:N5}, " +
