...
namespace FortitudeMarketsCore.Pricing.PQ.Publication
{
    public class PQPublisher<T> : IPQPublisher where T : IPQLevel0Quote
    {
        ...
        public PQPublisher(IPQServer<T> pqServer)
        {
        ...
        public void RegisterTickersWithServer(ISourceTickerPublicationConfigRepository tickersIdRef)
        {
            ...
            if (!shutdownFlag)
            {
                foreach (ISourceTickerPublicationConfig tickerRef in tickersIdRef)
                {
                    T picture = pqServer.Register(tickerRef.Ticker);
                    if (picture != null)
                    {
                        pictures.Add(tickerRef.Ticker, picture);
                    }
                }
            }
        }
        ...
        public void PublishReset(string ticker, DateTime exchangeTs, DateTime exchangeSentTs, DateTime adapterRecvTs)
        {
            if (pictures.TryGetValue(ticker, out var pqPicture))
            {
                ...
                if (pqPicture is IMutableLevel1Quote pq1)
                {
                    pq1.AdapterSentTime = exchangeSentTs;
                }
        ...
        public void Dispose()
        {
            ...
            if (pqServer != null)
            {
                foreach (KeyValuePair<string, T> pictureKvp in pictures)
                {
                    DateTime now = TimeContext.UtcNow;
                    var picture = pictureKvp.Value;
                    picture.ResetFields();
                    picture.SourceTime = now;
                    picture.ClientReceivedTime = now;
                    if (picture is IMutableLevel1Quote pq1)
                    {
                        pq1.AdapterSentTime = now;
                    }
