...
namespace FortitudeMarketsCore.Pricing.PQ.Publication
{
    public class PQServer<T> : IPQServer<T> where T : class, IPQLevel0Quote
    {
            ...
            Func<ISourceTickerQuoteInfo, T> quoteFactory = null)
        {
            ...
            {
                this.quoteFactory = ReflectionHelper.CtorBinder<ISourceTickerQuoteInfo, T>();
            }
            else
            {
        ...
        public T Register(string ticker)
        {
            ...
            {
                T ent;
                if (!entities.TryGetValue(tickerInfo.Id, out ent))
                {
        ...
        public void Publish(T quote)
        {
            ...
            T ent;
            if (entities.TryGetValue(quote.SourceTickerQuoteInfo.Id, out ent))
            {
                ent.Lock.Acquire();
                try
                {
        ...
        private void OnSnapshotRequest(ISocketSessionConnection cx, uint[] streamIDs)
        {
            ...
            {
                if (entities.TryGetValue(streamId, out var ent)) snapshotServer.Send(cx, ent);
            }
        }
