...
namespace FortitudeMarketsCore.Pricing.PQ.Quotes.SourceTickerInfo
{
    public class PQUniqueSourceTickerIdentifier : IPQUniqueSourceTickerIdentifier
    {
            ...
            IPQQuotePublicationPrecisionSettings quotePublicationPrecisionSettings = null)
        {
            ...
            if (!updatedOnly || IsIdUpdated)
            {
                yield return new PQFieldUpdate(PQFieldKeys.SourceTickerId, id);
            }
        }
        ...
        public virtual bool UpdateFieldString(PQFieldStringUpdate updates)
        {
            if(updates.Field.Id == PQFieldKeys.SourceTickerNames)
            {
                ...
                if (stringUpdt.DictionaryId == 0 && upsert)
                {
                    Source = updates.StringUpdate.Value;
                }
                if (stringUpdt.DictionaryId == 1 && upsert)
                {
                    Ticker = updates.StringUpdate.Value;
                }
                return true;
            }
            return false;
        ...
        public virtual void CopyFrom(IUniqueSourceTickerIdentifier sourceUniqueTickerId, CopyMergeFlags copyMergeFlags = CopyMergeFlags.Default)
        {
            if (sourceUniqueTickerId == null) return;

            if (!(sourceUniqueTickerId is PQUniqueSourceTickerIdentifier pqSrcTkrId))
            {
            ...
            {
                if (pqSrcTkrId.IsIdUpdated)
                {
                    Id = sourceUniqueTickerId.Id;
                }
                if (pqSrcTkrId.IsSourceUpdated)
                {
                    Source = sourceUniqueTickerId.Source;
                }
                if (pqSrcTkrId.IsTickerUpdated)
                {
                    Ticker = sourceUniqueTickerId.Ticker;
                }
                UpdatedFlags = pqSrcTkrId.UpdatedFlags;
            }
        }

        public virtual object Clone()
        {
            return new PQUniqueSourceTickerIdentifier(this);
        }
