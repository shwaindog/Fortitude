...
namespace FortitudeMarketsCore.Pricing.PQ.Quotes
{
    public class PQImplementationFactory : IPQImplementationFactory
    {
        ...
        public virtual T GetConcreteMapping<T>(ISourceTickerQuoteInfo quoteInfo) where T : IPQLevel0Quote
        {
            ...
            if (interfaceType == lvl0QuoteInterface) return (T)(object)new PQLevel0Quote(quoteInfo);
            if (interfaceType == lvl1QuoteInterface) return (T)(object)new PQLevel1Quote(quoteInfo);
            if (interfaceType == lvl2QuoteInterface) return (T)(object)new PQLevel2Quote(quoteInfo);
            if (interfaceType == lvl3QuoteInterface) return (T)(object)new PQLevel3Quote(quoteInfo);
