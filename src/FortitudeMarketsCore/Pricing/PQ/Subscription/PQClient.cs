...
namespace FortitudeMarketsCore.Pricing.PQ.Subscription
{
    public class PQClient : IDisposable
    {
        ...
        private readonly object unsubscribeSyncLock = new object();
        ...
        public IPQTickerFeedSubscriptionQuoteStream<T> GetQuoteStream<T>(ISourceTickerQuoteInfo sourceTickerQuoteInfo,
            int wholeMessagesPerReceive,
            uint syncRetryMsOverride = 60000) where T : class, IPQLevel0Quote
        {
            return GetQuoteStream<T>(sourceTickerQuoteInfo, null, wholeMessagesPerReceive, syncRetryMsOverride);
        }
            ...
            where T : class, IPQLevel0Quote
        {
            ...
            if (sourceTickerPublicationConfig != null)
            {
                ...
                if (quoteDeserializer != null)
                    throw new Exception("Subscription for " + sourceTickerQuoteInfo.Ticker + " on " + marketsServerConfig.Name +
                                        " already exists");
                ...
                updateClientFactory.RegisterSocketSubscriber(socketDescription,
                    ...
                    dispatcher, wholeMessagesPerReceive, factory, string.IsNullOrEmpty(alternativeMulticast)
        ...
        internal ISnapshotUpdatePricingServerConfig GetSourceServerConfig(string sourceName)
        {
            ...
            lock (feeds)
            {
                PQPricingServerSubscriptionContext context;
                if (feeds.TryGetValue(sourceName, out context))
        ...
        {
            IPQSnapshotClient snap = snapshotClientFactory.FindSocketSubscription(cfg);
            snap?.RequestSnapshots(streams);
        }

        private class PQPricingServerSubscriptionContext
        {
            public readonly List<IPQTickerFeedSubscription> Subscriptions = new List<IPQTickerFeedSubscription>();
