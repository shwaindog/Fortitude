...
namespace FortitudeMarketsCore.Trading.ORX.Subscription
{
    ...
    public class OrxTradingClient : OrxHistoricalTradesClient, ITradingFeedListener
    {
        ...
        protected readonly Dictionary<string, IOrder> ActiveOrders = new Dictionary<string, IOrder>();

        protected readonly Dictionary<string, IOrderAmend> AmendingOrders = new Dictionary<string, IOrderAmend>();
        ...
        : base(new OrxTradingClientMessaging(socksDispatcher, networkingController, tradingServerConfig.ServerConnections.First(), 
            ...
            defaultAccount)
        {
            ...
            Subscriber.RegisterDeserializer<OrxTickerMessage>((m, c, cx) => Subscriber.Logger.Info("Instrument update: " + m));
        ...
        public event Action<string, bool> FeedStatusUpdate
        {
            add
            {
                value(null, IsAvailable);
                lock (Statuses)
                {
                    foreach (var kv in Statuses) value(kv.Key, kv.Value);
                }
        ...
        private void HandleStatusFailure()
        {
            if (!IsAvailable)
            {
                ...
                {
                    order.Message = (MutableString)("Connection to " + ServerName + " lost while order still active");
        ...
        protected readonly Dictionary<string, bool> Statuses = new Dictionary<string, bool>();
        ...
        private void HandleSubmitReject(OrxSubmitReject message, object context, ISession cx)
        {
            ...
            order.Message = (MutableString)message.Reason;
        ...
        protected void HandleExecution(OrxExecutionUpdate update)
        {
            ...
            if (update.Execution.Quantity > 0)
            {
