...
{
    public SocketSessionConnection(ISocketSessionReceiver? socketSessionReceiver
        , ISocketSessionSender? socketSessionSender,
        Action<ISocketSessionConnection, string, int>? errorHandler)
    {
        SessionReceiver = socketSessionReceiver;
        if (socketSessionReceiver != null) socketSessionReceiver.Parent = this;
        SessionSender = socketSessionSender;
        if (socketSessionSender != null) socketSessionSender.Parent = this;
        Error = errorHandler;
    }
    ...
    {
        get => Previous;
        set => Previous = value as ISocketSessionConnection;
    }

    ISessionConnection? IDoublyLinkedListNode<ISessionConnection>.Next
    {
        get => Next;
        set => Next = value as ISocketSessionConnection;
    }
