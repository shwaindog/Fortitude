...
namespace FortitudeIO.Transports.NewSocketAPI.Publishing
{
    ...
    {
        private static IFLogger logger = FLoggerFactory.Instance.GetLogger(typeof(SocketSender));
        ...
        private readonly StaticRing<SocketSessionSender.SocketEncoder> encoders = 
            new StaticRing<SocketSessionSender.SocketEncoder>(1024,
        ...
        private volatile bool sendActive = false;
        ...
        private unsafe bool Send()
        {
            ...
            if (sentSize < 0) throw new Exception("Win32 error " +
                                                  directOSNetworkingApi.GetLastWin32Error() + " on send call");
            sentCursor += sentSize;
            if (sentCursor == toSendCursor)
            {
                sentCursor = toSendCursor = 0;
        ...
        {
            logger.Warn($"Error trying to send for {socketSocketSessionContext.ConversationDescription} got {message} and {exception}");
        }
    }
}
