...
namespace FortitudeIO.Transports.NewSocketAPI.Controls
{
    ...
    {
        private readonly object connSync = new object();
        ...
        private void ScheduleConnect(uint spanMs)
        {
            lock (connSync)
            {
                if (socketSessionContext.SocketConnection != null && socketSessionContext.SocketConnection.IsConnected || 
                    socketSessionContext.SocketSessionState == SocketSessionState.Connecting || triggerConnectNowSignal != null) return;
                socketSessionContext.OnSocketStateChanged(SocketSessionState.Connecting);
                if (spanMs > 0) triggerConnectNowSignal = parallelController.ScheduleWithEarlyTrigger(
        ...
        public void Connect()
        {
            ...
            lock (connSync)
            {
                ...
                for (ushort attemptNum = 0; attemptNum < maxAttempts; attemptNum++)
                {
                    ...
                    try
                    {
                        IOSSocket subscriberSocket = socketFactory.Create(
                        ...
                        socketSessionContext.OnConnected(new SocketConnection(connConfig.InstanceName, socketSessionContext.ConversationType,
        ...
        protected void Disconnect(bool inError)
        {
            lock (connSync)
            {
                if (socketSessionContext.SocketConnection?.IsConnected ?? false)
                {
                    if (!inError || socketSessionContext.SocketSessionState == SocketSessionState.Connecting) socketSessionContext.OnDisconnecting();
