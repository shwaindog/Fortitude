...
public class SocketDataLatencyLogger : ISocketDataLatencyLogger
{
    ...
    private static IFLogger Logger = FLoggerFactory.Instance.GetLogger("DiagnosticSettings");
    ...
    public SocketDataLatencyLogger(string name)
    {
        ...
        HierarchicalLoggingConfigurator<ICallStatsLogger>.Register(this);
    }

    public string FullNameOfLogger => GetType().FullName + "." + name;

    public Action<IHierarchicalLogger, string> SettingTranslation => dataDetectedToBeforeSocketReadCallStatsLogger.SettingTranslation;
