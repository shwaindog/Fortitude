...
public class SocketStreamMessageEncoderFactory : IStreamEncoderFactory
{
    ...
    public SocketStreamMessageEncoderFactory(IDictionary<uint, IMessageSerializer> serializerMap) => SerializerMap = serializerMap;
    ...
    public IMessageSerializer<T> MessageEncoder<T>(uint id) where T : class, IVersionedMessage => (IMessageSerializer<T>)SerializerMap[id];
