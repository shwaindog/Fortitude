...
namespace FortitudeIO.Protocols.ORX.Serialization.Deserialization
{
    public class OrxDeserializerLookup : IOrxDeserializerLookup
    {
        ...
        public OrxDeserializerLookup(IRecycler orxRecyclingFactory)
        {
        ...
        {
            for (byte i = fromVersion; i <= toVersion; i++)
            {
                ...
                if (!typeDictionary.ContainsKey(i))
                {
                    typeDictionary.Add(i, deserializer);
                }
            }
        }
