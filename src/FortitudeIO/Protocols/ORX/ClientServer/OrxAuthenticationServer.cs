...
namespace FortitudeIO.Protocols.ORX.ClientServer
{
    public class OrxAuthenticationServer
    {
        ...
        private readonly AutoResetEvent paAre = new AutoResetEvent(false);
        ...
        public OrxAuthenticationServer(OrxServerMessaging messagePublisher, byte currentVersion)
        {
            ...
            messagePublisher.StreamFromSubscriber.RegisterDeserializer<OrxLogonRequest>((ushort)AuthenticationMessages.LogonRequest, OnLogonRequest);
        ...
        {
            ISocketSessionConnection repoSess = repositorySession as SocketSessionConnection;
            ...
            if (OnAuthenticate != null
                && OnAuthenticate(repoSess, request.Login, request.Password, out var authData, out message))
            {
