...
namespace FortitudeIO.Topics.TopicRepository.TopicEndpoints
{
    ...
    public class TopicInstanceRegistry
    {
        ...
        private IDictionary<string, IDictionary<ConversationType, IInstanceEndpointSet>>
        ...
        private void CheckUpdateAndAlertTopics(ITopicInstanceUpdateMessage topicInstanceUpdateMessage, bool removeUnaddressed)
        {
            foreach (var topicUpdatedConnection in topicInstanceUpdateMessage.TopicsAffected)
            {
                ...
                foreach (var topicEndpointInfo in topicUpdatedConnection.Connections)
                {
                    if (listOfConnectionsOfThatType.Contains(topicEndpointInfo))
                    {
                        Logger.Warn("Already had this connection as created in TopicInstanceRegistry {0}", topicUpdatedConnection);
                    }
                    else
                    {
                        ...
                        foreach (var tei in listOfConnectionsOfThatType)
                        {
                            if (!tei.Equals(topicEndpointInfo) &&  tei.EquivalentEndpoint(topicEndpointInfo))
                            {
                                ...
                                listOfConnectionsOfThatType.Add(topicEndpointInfo);
                                OnTopicConnectionUpdate(EventType.Updated, topicUpdatedConnection.TopicName, topicEndpointInfo);
                                foundUpdate = true;
                                break;
                            }
                        }
                        if (!foundUpdate)
                        {
                            ...
                            listOfConnectionsOfThatType.Add(topicEndpointInfo);
                            OnTopicConnectionUpdate(EventType.Created, topicUpdatedConnection.TopicName, topicEndpointInfo);
                        }
                    }
                }
                ...
                if (removeUnaddressed && unmentionedConnections.Any())
                {
                    ...
                    {
                        OnTopicConnectionUpdate(EventType.Deleted, topicUpdatedConnection.TopicName, unmentionedConnection);
                    }
                }
            }
            OnTopicInstanceUpdate(topicInstanceUpdateMessage);
        ...
        private void CheckRemoveAndAlertTopics(ITopicInstanceUpdateMessage topicInstanceUpdateMessage)
        {
            foreach (var topicRemoveConnection in topicInstanceUpdateMessage.TopicsAffected)
            {
                ...
                {
                    CheckRemoveEndpoint(topicRemoveConnection.TopicName, listOfConnectionsOfThatType, topicEndpointInfo, topicRemoveConnection);
                }
            }
        ...
        protected void OnTopicConnectionUpdate(EventType updateType, string topic, ITopicEndpointInfo topicInstanceUpdateMessage)
        {
