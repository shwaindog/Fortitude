...
namespace FortitudeIO.Topics.TopicRepository.ORX
{
    public class OrxTopicClientConnectionDetails : ITopicInstanceConnectionDetails
    {
        ...
        [OrxMandatoryField(5)]
        public List<ITopicEndpointInfo> Connections { get; set; }

        public bool TopicHostSubscriptionTypeSameForInstance(ITopicInstanceConnectionDetails other)
        {
            return string.Equals(TopicName, other.TopicName) && string.Equals(Hostname, other.Hostname) &&
                   string.Equals(InstanceName, other.InstanceName) &&
                   ConversationType == other.ConversationType;
        }

        protected bool Equals(OrxTopicClientConnectionDetails other)
        {
            return TopicHostSubscriptionTypeSameForInstance(other);
        }
        ...
        {
            return $"{nameof(TopicName)}: {TopicName}, {nameof(Hostname)}: {Hostname}, " +
