...
namespace FortitudeIO.Transports.NewSocketAPI.Sockets
{
    public class ProtocolResolver
    {
            ...
            ISocketConnectionConfig socketConnectionConfig)
        {
            ...
            if (isUdp && conversationType == ConversationType.Publisher)
            {
                return SocketConversationProtocol.UDPPublisher;
            }
            if (isUdp && conversationType == ConversationType.Subscriber)
            {
                return SocketConversationProtocol.UDPSubscriber;
            }
