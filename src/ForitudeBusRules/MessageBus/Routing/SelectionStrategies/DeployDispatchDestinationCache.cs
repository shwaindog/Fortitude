...
{
    private readonly IMap<EventQueueType, DeployCacheEntry?> deployQueueTypeCache
        = new ConcurrentMap<EventQueueType, DeployCacheEntry?>();
        ...
        , RouteSelectionResult routeSelectionResult)
    {
        ...
        deployQueueTypeCache.AddOrUpdate(deploymentOptions.EventGroupType, new DeployCacheEntry(routeSelectionResult));
    }
    ...
    {
        var cacheEntry = deployQueueTypeCache.GetValue(eventQueueType);
        if (cacheEntry == null) return null;
        var updateEntry = cacheEntry.Value;
        return UpdateOrClearCache(eventQueueType, updateEntry) ? updateEntry.RouteSelectionResult : null;
    }

    private bool UpdateOrClearCache(EventQueueType eventQueueType, DeployCacheEntry checkForExpiry)
    {
        ...
        {
            deployQueueTypeCache.Remove(eventQueueType);
            return false;
        }

        deployQueueTypeCache.AddOrUpdate(eventQueueType, checkForExpiry);
        return true;
    }
