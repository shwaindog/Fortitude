...
namespace FortitudeMarketsCore.Pricing.PQ.DeltaUpdates
{
    public static class PQFieldConverters
    {
        ...
        {
            return new DateTime(DateTimeConstants.UnixEpochTicks + hoursFromUnixEpoch * TimeSpan.TicksPerHour +
                    (tensOfNanoSeconds / 10), DateTimeKind.Utc);
        }

        public static uint GetHoursFromUnixEpoch(this DateTime ts)
        {
            return (uint)((ts.Ticks - DateTimeConstants.UnixEpochTicks) / TimeSpan.TicksPerHour);
        }

        public static long GetSubHourComponent(this DateTime ts)
        {
            return ((ts.Ticks % TimeSpan.TicksPerHour) * 10);
        }
        public static long AppendUintToMakeLong(this byte highestNumberByte, uint lowestBytes)
        {
            long result = (long)highestNumberByte << 32;
            result |= lowestBytes;
            return result;
        }
