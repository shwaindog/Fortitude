...
namespace FortitudeMarketsCore.Pricing.PQ.DeltaUpdates
{
    public static class PQScaling
    {
        ...
        {
            return value * Factors[flag & FactorMask] * ((flag & NegativeMask) > 0 ? -1 : 1);
        }

        public static uint Scale(decimal value, byte flag)
        {
            return (uint)(Math.Abs(value) * Factors[16 - (flag & FactorMask)]);
        }
        ...
        {
            byte factor = 8;
            while (precision * Factors[factor] != Math.Round(precision * Factors[factor]) && factor <= 15)
            {
                factor++;
            }
