...
namespace FortitudeTests.FortitudeMarkets.Indicators.Pricing.PeriodSummaries.Construction;
...
public class HistoricalPeriodSummariesResolverRuleTests : OneOfEachMessageQueueTypeTestSetup
{
    ...
    private HistoricalPeriodParams fiveSecondsHistoricalPeriodParams;
    ...
    public async Task Setup()
    {
        ...
        fiveSecondsHistoricalPeriodParams = new HistoricalPeriodParams(tickerId5SPeriod, FiveSeconds, new TimeLength(TimeSpan.FromMinutes(30)));
        ...
        await indicatorRegistryStubRule.RegisterAndDeployGlobalService
            (ServiceType.HistoricalPricePeriodSummaryRetriever, pricePeriodSummariesRetrievalStubRule);
    }
    ...
    public async Task SummariesUpToDate_ResponseRequestForHistoricalGoesToRepoForUncached_ReceivesAllRequestedHistoricalSummaries()
    {
        ...
        var hasResult = await test30SHistoricalPeriodClient.InvokeResponseRequestOnTestClientQueue
            (new HistoricalPeriodResponseRequest(new BoundedTimeRange(historicalQuotesStartTime, historicalQuotesStartTime + TimeSpan.FromHours(1))));
    ...
    public async Task FromSubSummaryLargeHistory_FileUpToDate_ResponseRequestGeneratesResultsFromRepoSubSummaries()
    {
        ...
        var hasResult = await testClient.InvokeResponseRequestOnTestClientQueue
            (new HistoricalPeriodResponseRequest(new BoundedTimeRange(historicalQuotesStartTime, historicalQuotesLatestTime)));
        Assert.AreEqual(0, testClient.ReceivedToPersistEvents.Count);
        Assert.AreEqual(120, hasResult.Count);
    }
    ...
    private class TestHistoricalPeriodClient(TimeBoundaryPeriod period, SourceTickerIdentifier sourceTickerIdentifier) : Rule
    {
        ...
        public async ValueTask<List<PricePeriodSummary>> InvokeResponseRequestOnTestClientQueue
        ...
        public async ValueTask<List<PricePeriodSummary>> InvokeStreamRequestOnTestClientQueue
        ...
        private async ValueTask<List<PricePeriodSummary>> TestRequestResponseHandler
            (IBusRespondingMessage<HistoricalPeriodResponseRequest, List<PricePeriodSummary>> requestResponseMsg)
        {
            var historicalRequest = requestResponseMsg.Payload.Body();

            return await this.RequestAsync<HistoricalPeriodResponseRequest, List<PricePeriodSummary>>
                (sourceTickerIdentifier.HistoricalPeriodSummaryResponseRequest(period), historicalRequest);
        }

        private async ValueTask<List<PricePeriodSummary>> TestStreamRequestHandler
            (IBusRespondingMessage<HistoricalPeriodStreamRequest, List<PricePeriodSummary>> streamRequestMsg)
        {
            ...
            var expectResults = await this.RequestAsync<HistoricalPeriodStreamRequest, bool>
                (sourceTickerIdentifier.HistoricalPeriodSummaryStreamRequest(period), historicalRequest);
