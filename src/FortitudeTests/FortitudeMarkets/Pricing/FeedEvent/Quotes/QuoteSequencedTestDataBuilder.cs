...
public class QuoteSequencedTestDataBuilder
{
    ...
    private void SetupLevel1Quote(IMutableLevel1Quote? level1Quote, uint batchId)
    {
        ...
        if (level1Quote is IMutablePublishableLevel1Quote pubL1Quote)
        {
            ...
            switch (pubL1Quote.SummaryPeriod)
            {
                case null when level1Quote is PQPublishableLevel1Quote: pubL1Quote.SummaryPeriod = new PQPricePeriodSummary(); break;
                case null:                                              pubL1Quote.SummaryPeriod = new PricePeriodSummary(); break;
            }

            InitalizePeriodSummary(pubL1Quote.SummaryPeriod, batchId);
        }
    }
