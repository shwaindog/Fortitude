...
public class PQRecentlyTradedTests
{
      ...
      ,  PQFieldFlags priceScale = (PQFieldFlags)1,  PQFieldFlags volumeScale = (PQFieldFlags)6)
    {
        for (var i = 0; i < recentlyTraded.Count; i++)
        {
            ...
            Assert.AreEqual(new PQFieldUpdate(PQQuoteFields.LastTradedTickTrades, depthId, PQTradingSubFieldKeys.LastTradedAtPrice, lastTrade.TradePrice, priceScale),
            ...
            Assert.AreEqual(new PQFieldUpdate(PQQuoteFields.LastTradedTickTrades, depthId, PQTradingSubFieldKeys.LastTradedTradeTimeDate,
            ...
            Assert.AreEqual(new PQFieldUpdate(PQQuoteFields.LastTradedTickTrades, depthId, PQTradingSubFieldKeys.LastTradedTradeSub2MinTime, subHourBase, extended)
                          , PQTickInstantTests.ExtractFieldUpdateWithId
                                (checkFieldUpdates, PQQuoteFields.LastTradedTickTrades, depthId, PQTradingSubFieldKeys.LastTradedTradeSub2MinTime, extended),
            ...
            if (lastTrade is IPQLastPaidGivenTrade pqPaidGivenTrade)
            {
                ...
                Assert.AreEqual(new PQFieldUpdate(PQQuoteFields.LastTradedTickTrades, depthId, PQTradingSubFieldKeys.LastTradedBooleanFlags, (uint)lastTradedBoolFlags),
                                PQTickInstantTests.ExtractFieldUpdateWithId
                                    (checkFieldUpdates, PQQuoteFields.LastTradedTickTrades, depthId, PQTradingSubFieldKeys.LastTradedBooleanFlags),
                                $"For asklayer {lastTrade.GetType().Name} level {i} with these fields\n{string.Join(",\n", checkFieldUpdates)}");
            }
            ...
            if (lastTrade is IPQLastTraderPaidGivenTrade pqTraderPaidGivenTrade)
            {
                ...
                Assert.AreEqual(new PQFieldUpdate(PQQuoteFields.LastTradedTickTrades, depthId, PQTradingSubFieldKeys.LastTradedTraderId, lastTradedTraderId),
                                PQTickInstantTests.ExtractFieldUpdateWithId(checkFieldUpdates, PQQuoteFields.LastTradedTickTrades, depthId, 
                                                                            PQTradingSubFieldKeys.LastTradedTraderId),
                                $"For asklayer {lastTrade.GetType().Name} level {i} with these fields\n{string.Join(",\n", checkFieldUpdates)}");
            }
        }
    }
