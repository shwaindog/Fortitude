        ...
        IPQOrdersPriceVolumeLayer ordersPvl, bool isAskSide, int depthIndex, IPQNameIdLookupGenerator nameIdLookup, PQFieldFlags volumeScale)
    {
        ...
        for (var j = 0; j < ordersPvl.OrdersCount; j++)
        {
            ...
            if (anonOrderLayerInfo is IPQCounterPartyOrderLayerInfo cpOrderLayerInfo)
            {
                ...
                Assert.AreEqual(new PQFieldUpdate(PQQuoteFields.LayerOrders, depthId, PQTradingSubFieldKeys.OrderExternalCounterPartyNameId, orderIndex, orderCpNameId)
                              , orderCpIdFu,
                                $"For {ordersPvl.GetType().Name} at [{depthIndex}][{j}] with these fields\n{string.Join(",\n", checkFieldUpdates)}");

                var orderTraderId = (uint)nameIdLookup[cpOrderLayerInfo.ExternalTraderName];
                var orderTraderIdFu
                    = ExtractFieldUpdateWithId(checkFieldUpdates, PQQuoteFields.LayerOrders, depthId, PQTradingSubFieldKeys.OrderExternalTraderNameId, orderIndex
                                             , orderTraderId);
                Assert.AreEqual(new PQFieldUpdate(PQQuoteFields.LayerOrders, depthId, PQTradingSubFieldKeys.OrderExternalTraderNameId, orderIndex, orderTraderId)
                              , orderTraderIdFu,
                                $"For {ordersPvl.GetType().Name} at [{depthIndex}][{j}] with these fields\n{string.Join(",\n", checkFieldUpdates)}");
                return;
            }
