...
public class PQCounterPartyOrderLayerInfoTests
{
    ...
    )
    {
        ...
        var expectedOrderInfo
            = new PQFieldUpdate(PQQuoteFields.LayerOrders, PQTradingSubFieldKeys.OrderExternalCounterPartyNameId, dictId);
        ...
        if (l2QNotNull)
        {
            l2QUpdates =
                (from update in l2Quote!.GetDeltaUpdateFields(testDateTime, StorageFlags.Snapshot, precisionSettings)
                    where update is { Id: PQQuoteFields.LayerOrders, TradingSubId: PQTradingSubFieldKeys.OrderExternalCounterPartyNameId }
        ...
        if (olNotNull)
        {
            olUpdates =
                (from update in ordersLayer!.GetDeltaUpdateFields(testDateTime, StorageFlags.Snapshot, precisionSettings)
                    where update is { Id: PQQuoteFields.LayerOrders, TradingSubId: PQTradingSubFieldKeys.OrderExternalCounterPartyNameId } && update.AuxiliaryPayload == orderIndex
                    select update).ToList();
    ...
    )
    {
        ...
        var expectedOrderInfo
            = new PQFieldUpdate(PQQuoteFields.LayerOrders, PQTradingSubFieldKeys.OrderExternalTraderNameId, dictId);
        var expectedLayer     = expectedOrderInfo.WithAuxiliary(orderIndex);
        var expectedBookSide  = expectedLayer.WithDepth(depthNoSide);
        var expectedOrderBook = expectedBookSide.WithDepth(depthWithSide);
        Assert.AreEqual(expectedOrderInfo, orderInfoUpdates[0]);
        ...
        if (l2QNotNull)
        {
            l2QUpdates =
                (from update in l2Quote!.GetDeltaUpdateFields(testDateTime, StorageFlags.Snapshot, precisionSettings)
                    where update is { Id: PQQuoteFields.LayerOrders, TradingSubId: PQTradingSubFieldKeys.OrderExternalTraderNameId }
                       && update.DepthId == depthWithSide && update.AuxiliaryPayload == orderIndex
                    select update).ToList();
            Assert.AreEqual(1, l2QUpdates.Count);
            Assert.AreEqual(expectedOrderBook, l2QUpdates[0]);
        ...
        if (olNotNull)
        {
            olUpdates =
                (from update in ordersLayer!.GetDeltaUpdateFields(testDateTime, StorageFlags.Snapshot, precisionSettings)
                    where update is { Id: PQQuoteFields.LayerOrders, TradingSubId: PQTradingSubFieldKeys.OrderExternalTraderNameId } && update.AuxiliaryPayload == orderIndex
                    select update).ToList();
            Assert.AreEqual(1, olUpdates.Count);
            Assert.AreEqual(expectedLayer, olUpdates[0]);
