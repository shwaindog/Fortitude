...
public class PQLastPaidGivenTradeTests
{
    ...
    public void EmptyLt_TradeVolumeChanged_ExpectedPropertyUpdatedDeltaUpdatesAffected()
    {
        ...
        var expectedFieldUpdate = new PQFieldUpdate(PQQuoteFields.LastTradedTickTrades, PQTradingSubFieldKeys.LastTradedOrderVolume, expectedVolume, (PQFieldFlags)6);
        ...
        Assert.AreEqual(1, sourceUpdates.Count);
        expectedFieldUpdate = new PQFieldUpdate(PQQuoteFields.LastTradedTickTrades, PQTradingSubFieldKeys.LastTradedOrderVolume, nextExpectedVolume, (PQFieldFlags)6);
        Assert.AreEqual(expectedFieldUpdate, sourceUpdates[0]);

        emptyLt.HasUpdates = false;
        sourceUpdates = (from update in emptyLt.GetDeltaUpdateFields(testDateTime, StorageFlags.Snapshot)
            where update.TradingSubId == PQTradingSubFieldKeys.LastTradedOrderVolume
    ...
    public void EmptyLt_WasGivenChanged_ExpectedPropertiesUpdatedDeltaUpdatesAffected()
    {
        ...
        var expectedLayerField = new PQFieldUpdate(PQQuoteFields.LastTradedTickTrades, PQTradingSubFieldKeys.LastTradedBooleanFlags, (uint)LastTradeBooleanFlags.WasGiven);
    ...
    public void EmptyLt_WasPaidChanged_ExpectedPropertiesUpdatedDeltaUpdatesAffected()
    {
        ...
        var sourceLayerUpdates = emptyLt.GetDeltaUpdateFields(testDateTime, StorageFlags.Update).ToList();
        Assert.AreEqual(1, sourceLayerUpdates.Count);
        var expectedLayerField = new PQFieldUpdate(PQQuoteFields.LastTradedTickTrades, PQTradingSubFieldKeys.LastTradedBooleanFlags, (uint)LastTradeBooleanFlags.WasPaid);
        Assert.AreEqual(expectedLayerField, sourceLayerUpdates[0]);
        ...
        IPQLastPaidGivenTrade lt)
    {
        ...
        Assert.AreEqual(new PQFieldUpdate(PQQuoteFields.LastTradedTickTrades, PQTradingSubFieldKeys.LastTradedBooleanFlags,
                                          (uint)expectedFlag),
                        PQTickInstantTests.ExtractFieldUpdateWithId(checkFieldUpdates,
                                                                    PQQuoteFields.LastTradedTickTrades, PQTradingSubFieldKeys.LastTradedBooleanFlags), $"For asklayer {lt.GetType().Name}");
    }
