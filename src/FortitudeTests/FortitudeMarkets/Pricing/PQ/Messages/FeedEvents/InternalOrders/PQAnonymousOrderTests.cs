...
public class PQAnonymousOrderTests
{
    ...
    public void NewPqAoli_NewFromCloneInstance_PropertiesInitializedAsExpected()
    {
        ...
        Assert.AreEqual(populatedAoli.OrderDisplayVolume, fromNonPQInstance.OrderDisplayVolume);
    ...
    )
    {
        ...
        Assert.AreEqual(0, anonOrderInfo.GetDeltaUpdateFields(testDateTime, PQMessageFlags.Update).Count());
        ...
        orderInfoUpdates =
            (from update in anonOrderInfo.GetDeltaUpdateFields(testDateTime, PQMessageFlags.Snapshot, precisionSettings)
                where update is { Id: PQFeedFields.QuoteLayerOrders, OrdersSubId: PQOrdersSubFieldKeys.OrderId }
                select update).ToList();
        Assert.AreEqual(1, orderInfoUpdates.Count);
        Assert.AreEqual(expectedOrderInfo, orderInfoUpdates[0]);
    ...
    )
    {
        ...
        Assert.IsTrue(anonOrderInfo.HasUpdates);
        Assert.AreEqual(expectedOrderVolume, anonOrderInfo.OrderDisplayVolume);
        ...
        if (l2QNotNull)
        {
            ...
            var foundLayer =
                (IPQOrdersPriceVolumeLayer)(isBid ? newEmpty.BidBook : newEmpty.AskBook)[bookDepth]!;
            var foundAnonOrderInfo = foundLayer[orderIndex]!;
            Assert.AreEqual(expectedOrderVolume, foundAnonOrderInfo.OrderDisplayVolume);
    ...
    public void EmptyPqAoli_OrderVolumeChanged_ExpectedPropertiesUpdated()
    {
        ...
        Assert.AreEqual(changedOrderVolume, emptyAoli.OrderDisplayVolume);
    ...
    public void PopulatedPqAoli_Reset_ReturnsReturnsLayerToEmpty()
    {
        ...
        Assert.AreEqual(OrderVolume, populatedAoli.OrderDisplayVolume);
    ...
    public void FullyPopulatedPqAoli_Clone_ClonedInstanceEqualsOriginal()
    {
        ...
        var cloned4 = populatedAoli.Clone();
    ...
    public void FullyPopulatedTli_ToString_ReturnsNameAndValues()
    {
        ...
        Assert.IsTrue(toString.Contains($"{nameof(IAnonymousOrder.OrderDisplayVolume)}: {populatedAoli.OrderDisplayVolume:N2}"));
