...
public class PQSourcePriceVolumeLayerTests
{
    ...
    public void SetUp()
    {
        ...
        emptyPvl     = new PQSourcePriceVolumeLayer(nameIdLookup.Clone());
    ...
    )
    {
        ...
        Assert.AreEqual(0, sourcePvl.GetDeltaUpdateFields(testDateTime, PQMessageFlags.Update).Count());
        ...
        var expectedLayer
            = new PQFieldUpdate(PQFeedFields.QuoteLayerSourceId, dictId);
        ...
        if (l2QNotNull)
        {
            l2QUpdates =
                (from update in l2Quote!.GetDeltaUpdateFields(testDateTime, PQMessageFlags.Snapshot, precisionSettings)
                    where update is { Id: PQFeedFields.QuoteLayerSourceId }
        ...
        orderInfoUpdates =
            (from update in sourcePvl.GetDeltaUpdateFields(testDateTime, PQMessageFlags.Snapshot, precisionSettings)
                where update is { Id: PQFeedFields.QuoteLayerSourceId }
                select update).ToList();
        Assert.AreEqual(1, orderInfoUpdates.Count);
        Assert.AreEqual(expectedLayer, orderInfoUpdates[0]);
        ...
        foreach (var stringUpdate in sourcePvl.GetStringUpdates(testDateTime, PQMessageFlags.Snapshot))
        {
    ...
    )
    {
        ...
        var expectedBooleanFlags = expectedExecutable ? LayerBooleanValues.Executable : LayerBooleanValues.None;
        ...
        Assert.AreEqual(1, orderInfoUpdates.Count);
        var expectedLayer
            = new PQFieldUpdate(PQFeedFields.QuoteLayerBooleanFlags, (uint)expectedBooleanFlags);
        var expectedBookSide  = expectedLayer.WithDepth(depthNoSide);
        var expectedOrderBook = expectedBookSide.WithDepth(depthWithSide);
        Assert.AreEqual(expectedLayer, orderInfoUpdates[0]);
        if (bsNotNull) Assert.AreEqual(expectedBookSide, bsUpdates[0]);
        ...
        if (l2QNotNull)
        {
            l2QUpdates =
                (from update in l2Quote!.GetDeltaUpdateFields(testDateTime, PQMessageFlags.Snapshot, precisionSettings)
                    where update is { Id: PQFeedFields.QuoteLayerBooleanFlags }
                       && update.DepthId == depthWithSide
                    select update).ToList();
            Assert.AreEqual(1, l2QUpdates.Count);
            Assert.AreEqual(expectedOrderBook, l2QUpdates[0]);
        ...
        PQFieldFlags priceScale = (PQFieldFlags)1, PQFieldFlags volumeScale = (PQFieldFlags)6)
    {
        ...
        Assert.AreEqual(new PQFieldUpdate(PQFeedFields.QuoteLayerSourceId, depthId, pvl.SourceId),
                        PQTickInstantTests.ExtractFieldUpdateWithId(checkFieldUpdates, PQFeedFields.QuoteLayerSourceId, depthId),
                        $"For {pvl.GetType().Name} at {bookIndex} with these fields\n{string.Join(",\n", checkFieldUpdates)}");

        Assert.AreEqual(new PQFieldUpdate(PQFeedFields.QuoteLayerBooleanFlags, depthId,
                                          pvl.Executable ? LayerBooleanValues.Executable.JustLayerBooleanValuesMask() : 0u),
                        PQTickInstantTests.ExtractFieldUpdateWithId(checkFieldUpdates, PQFeedFields.QuoteLayerBooleanFlags, depthId),
                        $"For {pvl.GetType().Name} at {bookIndex} with these fields\n{string.Join(",\n", checkFieldUpdates)}");
    }
