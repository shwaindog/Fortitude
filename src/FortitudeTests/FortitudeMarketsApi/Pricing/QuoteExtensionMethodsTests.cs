...
namespace FortitudeTests.FortitudeMarketsApi.Pricing
{
    ...
    {
        private ILevel3Quote originalQuote;
        ...
        private readonly SourceTickerQuoteInfo originalSourceTickerQuoteInfo = new SourceTickerQuoteInfo(1, 
            OriginalQuoteExchangeName, OriginalQuoteTickerName, 20, 0.0001m, 1m, 1000000m, 1m);
        private readonly DateTime originalQuoteExchangeTime = new DateTime(2015, 11, 17, 22, 07, 23);
        private readonly DateTime originalQuoteAdapterTime = new DateTime(2015, 08, 09, 22, 07, 24);
        private readonly DateTime originalQuoteClientReceiveTime = DateTime.Parse("2015-08-09 10:06:23.123456");
        private readonly DateTime originalQuoteBidDateTime = new DateTime(2015, 10, 16, 16, 44, 11);
        private readonly DateTime originalQuoteAskDateTime = new DateTime(2015, 10, 16, 16, 44, 12);
        ...
        public void TestSetup()
        {
            originalQuote = new Level3PriceQuote(originalSourceTickerQuoteInfo, originalQuoteExchangeTime, false,
                ...
                new OrderBook(new[] { new PriceVolumeLayer(OriginalBidTopPrice, OriginalBidTopVolume),
                    new PriceVolumeLayer(1.1122m, 20000) }),
                true,
                new OrderBook(new[] { new PriceVolumeLayer(OriginalAskTopPrice, OriginalAskTopVolume),
                    new PriceVolumeLayer(1.1126m, 40000) }), true, null, 0, 0, new DateTime(2017, 12, 29, 21, 0, 0));
        }
        ...
        public void IdenticalExceptExchangeNameQuotes_Diff_ReturnsExchangeNameDifferent()
        {
            ...
            SourceTickerQuoteInfo newSourceTickerQuoteInfo = new SourceTickerQuoteInfo(1, newExchangerName, 
            ...
            SourceTickerQuoteInfo updatedNewQuoteInfo = (SourceTickerQuoteInfo)q2.SourceTickerQuoteInfo;
            NonPublicInvocator.SetAutoPropertyInstanceField(updatedNewQuoteInfo, 
                (SourceTickerQuoteInfo stqi) => stqi.Source, OriginalQuoteExchangeName);
        ...
        public void IdenticalExceptTickerQuotes_Diff_ReturnsTickerDifferent()
        {
            ...
            SourceTickerQuoteInfo newSourceTickerQuoteInfo = new SourceTickerQuoteInfo(1, OriginalQuoteExchangeName, 
            ...
            SourceTickerQuoteInfo updatedNewQuoteInfo = (SourceTickerQuoteInfo)q2.SourceTickerQuoteInfo;
            NonPublicInvocator.SetAutoPropertyInstanceField(updatedNewQuoteInfo, 
                (SourceTickerQuoteInfo stqi) => stqi.Ticker, OriginalQuoteTickerName);
            differences = originalQuote.DiffQuotes(q2);
            Console.Out.Write(differences);
            Assert.AreEqual("", differences);
        }
        ...
        public void IdenticalExceptExchangeTimeQuotes_Diff_ReturnsExchangeTimeDifferent()
        {
            ...
            Assert.IsTrue(differences.Contains(nameof(q2.SourceTime)) 
                && differences.Contains(originalQuoteExchangeTime.ToString(DateTimeConstants.FullDateTimeFormatToMicro)) 
                && differences.Contains(newExchangeDateTime.ToString(DateTimeConstants.FullDateTimeFormatToMicro)));
        }
        ...
        public void IdenticalExceptAdapterTimeQuotes_Diff_ReturnsAdapterTimeDifferent()
        {
            ...
            NonPublicInvocator.SetAutoPropertyInstanceField(q2, 
                (Level3PriceQuote pq) => pq.AdapterSentTime, newAdapterDateTime);
            var differences = originalQuote.DiffQuotes(q2);
            Console.Out.WriteLine(differences);
            Assert.IsTrue(differences.Contains(nameof(q2.AdapterSentTime)) 
                && differences.Contains(originalQuoteAdapterTime.ToString(DateTimeConstants.FullDateTimeFormatToMicro)) 
                && differences.Contains(newAdapterDateTime.ToString(DateTimeConstants.FullDateTimeFormatToMicro)));
        }
        ...
        public void IdenticalExceptReceivedTimeQuotes_Diff_ReturnsReceiveTimeDifferent()
        {
            ...
            NonPublicInvocator.SetAutoPropertyInstanceField(q2, 
                (Level3PriceQuote pq) => pq.ClientReceivedTime, newClientReceivedDateTime);
            var differences = originalQuote.DiffQuotes(q2, true);
            Console.Out.WriteLine(differences);
            Assert.IsTrue(differences.Contains(nameof(q2.ClientReceivedTime)) 
                && differences.Contains(originalQuoteClientReceiveTime
                .ToString(DateTimeConstants.FullDateTimeFormatToMicro)) 
                && differences.Contains(newClientReceivedDateTime
                .ToString(DateTimeConstants.FullDateTimeFormatToMicro)));
            differences = originalQuote.DiffQuotes(q2);
            Console.Out.WriteLine(differences);
            Assert.AreEqual("", differences);
        ...
        public void IdenticalExceptBidTopQuotes_Diff_ReturnsBidTopDifferent()
        {
            ...
            var differences = originalQuote.DiffQuotes(q2);
            Console.Out.WriteLine(differences);
            string expectedOriginal = OriginalBidTopPrice.ToString(PricingConstants.UniversalPriceFormating);
            string expectedNew = newBidTopPrice.ToString(PricingConstants.UniversalPriceFormating);
        ...
        public void IdenticalExceptAskTopQuotes_Diff_ReturnsAskTopDifferent()
        {
            ...
            string expectedOriginal = OriginalAskTopPrice.ToString(PricingConstants.UniversalPriceFormating);
            string expectedNew = newAskTopPrice.ToString(PricingConstants.UniversalPriceFormating);
        ...
        public void IdenticalExceptBidBookQuotes_Diff_ReturnsBidBookDifferent()
        {
            ...
            NonPublicInvocator.SetAutoPropertyInstanceField(q2, (Level3PriceQuote pq) => pq.BidBook, null);
            ...
            Assert.IsTrue(differences.Contains(nameof(q2.BidBook))
                && differences.Contains("[ 0]")
                && differences.Contains("[ 1]")
                && differences.Contains("not null")
                && differences.Select((c, i) => differences.Substring(i))
                .Count(sub => sub.StartsWith("=null")) == 3
                && differences.Select((c, i) => differences.Substring(i))
                .Count(sub => sub.StartsWith("PriceVolumeLayer")) == 2);
        ...
        {
            var q2 = originalQuote.Clone();
            var newAskTopDateTime = new OrderBook((IReadOnlyList<IPriceVolumeLayer>)null);
            ...
            Assert.IsTrue(differences.Contains(nameof(q2.AskBook))
                ...
                && differences.Select((c, i) => differences.Substring(i))
                .Count(sub => sub.StartsWith("PriceVolumeLayer")) == 2);
            NonPublicInvocator.SetAutoPropertyInstanceField(q2, (Level3PriceQuote pq) => pq.AskBook, null);
            ...
            Assert.IsTrue(differences.Contains(nameof(q2.AskBook))
                ...
                && differences.Contains("[ 1]")
                && differences.Contains("not null")
                && differences.Select((c, i) => differences.Substring(i)).Count(sub => sub.StartsWith("=null")) == 3
                && differences.Select((c, i) => differences.Substring(i))
                .Count(sub => sub.StartsWith("PriceVolumeLayer")) == 2);
            ...
            Assert.IsTrue(differences.Contains(nameof(q2.AskBook))
                && differences.Select((c, i) => differences.Substring(i))
                .Count(sub => sub.StartsWith(expectedOriginal)) == 1
                && differences.Select((c, i) => differences.Substring(i)).Count(sub => sub.StartsWith(expectedNew)) == 1
                && differences.Contains("PriceVolumeLayer"));
        }
    }
}
