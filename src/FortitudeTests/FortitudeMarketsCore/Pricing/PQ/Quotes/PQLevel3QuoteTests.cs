...
using FortitudeTests.FortitudeMarketsCore.Pricing.PQ.LastTraded;
...
namespace FortitudeTests.FortitudeMarketsCore.Pricing.PQ.Quotes
{
    ...
    {
        private ISourceTickerQuoteInfo noRecentlyTradedSrcTkrQtInfo;
        private ISourceTickerQuoteInfo simpleRecentlyTradedSrcTkrQtInfo;
        private ISourceTickerQuoteInfo paidGivenVolumeRecentlyTradedSrcTkrQtInfo;
        private ISourceTickerQuoteInfo traderPaidGivenVolumeRecentlyTradedSrcTkrQtInfo;
        private PQLevel3Quote noRecentlyTradedEmptyQuote;
        private PQLevel3Quote noRecentlyTradedFullyPopulatedQuote;
        private PQLevel3Quote simpleRecentlyTradedEmptyQuote;
        private PQLevel3Quote simpleRecentlyTradedFullyPopulatedQuote;
        private PQLevel3Quote paidGivenVolumeRecentlyTradedEmptyQuote;
        private PQLevel3Quote paidGivenVolumeRecentlyTradedFullyPopulatedQuote;
        private PQLevel3Quote traderPaidGivenVolumeRecentlyTradedEmptyQuote;
        private PQLevel3Quote traderPaidGivenVolumeRecentlyTradedFullyPopulatedQuote;

        private IList<PQLevel3Quote> allFullyPopulatedQuotes;
        private IList<PQLevel3Quote> allEmptyQuotes;
        ...
        public void SetUp()
        {
            ...
            {
                noRecentlyTradedEmptyQuote, simpleRecentlyTradedEmptyQuote,
                paidGivenVolumeRecentlyTradedEmptyQuote, traderPaidGivenVolumeRecentlyTradedEmptyQuote
            };
        ...
        {
            AssertLastTradeTypeIsExpected(typeof(PQLastTrade), simpleRecentlyTradedEmptyQuote,
        ...
        {
            AssertLastTradeTypeIsExpected(typeof(PQLastPaidGivenTrade), paidGivenVolumeRecentlyTradedEmptyQuote,
                paidGivenVolumeRecentlyTradedFullyPopulatedQuote);
        }
        ...
        {
            AssertLastTradeTypeIsExpected(typeof(PQLastTraderPaidGivenTrade), 
                traderPaidGivenVolumeRecentlyTradedEmptyQuote,
                traderPaidGivenVolumeRecentlyTradedFullyPopulatedQuote);
        }
        ...
        public void EmptyLevel3Quote_BatchIdChanged_ExpectedPropertiesUpdatedDeltaUpdatesAffected()
        {
            ...
            Assert.AreEqual(2, noRecentlyTradedEmptyQuote.GetDeltaUpdateFields(testDateTime, UpdateStyle.Updates)
                .Count());
            ...
            var level3QuoteUpdates = noRecentlyTradedEmptyQuote.GetDeltaUpdateFields(testDateTime, 
                UpdateStyle.Updates).ToList();
            ...
            Assert.IsFalse(noRecentlyTradedEmptyQuote.HasUpdates);
            Assert.IsTrue(noRecentlyTradedEmptyQuote.GetDeltaUpdateFields(testDateTime, UpdateStyle.Updates)
                .IsNullOrEmpty());

            noRecentlyTradedEmptyQuote.IsBatchIdUpdated = true;
            level3QuoteUpdates =
            (from update in noRecentlyTradedEmptyQuote.GetDeltaUpdateFields(testDateTime, UpdateStyle.Updates)
        ...
        public void EmptyLevel3Quote_ValueDateChanged_ExpectedPropertiesUpdatedDeltaUpdatesAffected()
        {
            ...
            var level3QuoteUpdates = noRecentlyTradedEmptyQuote.GetDeltaUpdateFields(testDateTime, 
                UpdateStyle.Updates).ToList();
            Assert.AreEqual(3, level3QuoteUpdates.Count);
            var expectedLayerField = new PQFieldUpdate(PQFieldKeys.ValueDate, 
                expectedValueDate.GetHoursFromUnixEpoch());
            Assert.AreEqual(expectedLayerField, level3QuoteUpdates[2]);
        ...
        public void AllLevel3QuoteTypes_LastTradePriceChanged_ExpectedPropertiesUpdatedDeltaUpdatesAffected()
        {
            foreach (var emptyQuote in allEmptyQuotes)
            {
                ...
                for (int i = 0; i < PQFieldKeys.SingleByteFieldIdMaxPossibleLastTrades; i++)
                {
                    ...
                    Assert.AreEqual(0, lastTrade.GetDeltaUpdateFields(testDateTime, UpdateStyle.Updates)
                        .Count());
                    ...
                    var lastTradeUpdates = lastTrade.GetDeltaUpdateFields(testDateTime, UpdateStyle.Updates)
                        .ToList();
                    Assert.AreEqual(1, lastTradeUpdates.Count);
                    var expectedLastTradeField = new PQFieldUpdate(PQFieldKeys.LastTradePriceOffset,
                        expectedPrice, 1);
                    var expectedQuoteLastTradeField = new PQFieldUpdate(
                        (byte)(PQFieldKeys.LastTradePriceOffset + i),
                        expectedLastTradeField.Value, 1);
        ...
        public void AllLevel3QuoteTypes_LastTradeTimeChanged_ExpectedPropertiesUpdatedDeltaUpdatesAffected()
        {
            foreach (var emptyQuote in allEmptyQuotes)
            {
                ...
                for (int i = 0; i < PQFieldKeys.SingleByteFieldIdMaxPossibleLastTrades; i++)
                {
                    ...
                    var expectedLastTradeHoursField = new PQFieldUpdate(PQFieldKeys.LastTradeTimeHourOffset,
                        hoursFromEpoch);
                    var expectedLastTradeSubHoursField = new PQFieldUpdate(PQFieldKeys.LastTradeTimeSubHourOffset,
                        subHourBase, flag);
                    ...
                    var expectedQuoteLastTradeTimeHoursField = new PQFieldUpdate(
                        (byte)(PQFieldKeys.LastTradeTimeHourOffset + i), hoursFromEpoch);
                    var expectedQuoteLastTradeTimeSubHoursField = new PQFieldUpdate(
                        (byte)(PQFieldKeys.LastTradeTimeSubHourOffset + i), subHourBase, flag);
                    ...
                    Assert.IsTrue(emptyQuote.HasUpdates);
                    var lastTradeJustSubHourUpdate = lastTrade.GetDeltaUpdateFields(testDateTime, UpdateStyle.Updates)
                        .ToList();
                    ...
                    var quoteSubHourUpdates = emptyQuote.GetDeltaUpdateFields(testDateTime, UpdateStyle.Updates)
                        .ToList();
        ...
        public void AllLevel3QuoteTypes_LastTradeVolumeChanged_ExpectedPropertiesUpdatedDeltaUpdatesAffected()
        {
            foreach (var emptyQuote in allEmptyQuotes)
            {
                ...
                for (int i = 0; i < PQFieldKeys.SingleByteFieldIdMaxPossibleLastTrades; i++)
                {
                    ...
                    var expectedQuoteLastTradeField = new PQFieldUpdate(
                        (byte)(PQFieldKeys.LastTradeVolumeOffset + i),
                        expectedLastTradeField.Value, 6);
                    Assert.AreEqual(expectedLastTradeField, lastTradeUpdates[0]);
                    Assert.AreEqual(expectedQuoteLastTradeField, quoteUpdates[2]);
            ...
            foreach (var emptyQuote in allEmptyQuotes)
            {
                ...
                for (int i = 0; i < PQFieldKeys.SingleByteFieldIdMaxPossibleLastTrades; i++)
                {
                    ...
                    var lastTradeUpdates = lastTrade.GetDeltaUpdateFields(testDateTime, UpdateStyle.Updates)
                        .ToList();
                    Assert.AreEqual(1, lastTradeUpdates.Count);
                    var expectedLastTradeField = new PQFieldUpdate(PQFieldKeys.LastTraderIdOffset,
                        lastTrade.TraderId);
                    var expectedQuoteLastTradeField = new PQFieldUpdate(
                    ...
                    Assert.AreEqual(expectedLastTradeField, lastTradeUpdates[0]);
                    Assert.AreEqual(expectedQuoteLastTradeField, quoteUpdates[2]);
                    var stringUpdates = emptyQuote.GetStringUpdates(testDateTime, UpdateStyle.Updates)
                        .ToList();
                    var expectedStringUpdates = new PQFieldStringUpdate
                    {
                        ...
                        StringUpdate = new PQStringUpdate
                        {
                            ...
                            DictionaryId = lastTrade.TraderNameIdLookup[lastTrade.TraderName],
        ...
        public void AllFullyPopulatedQuotes_HasUpdatesSetFalse_RemovesUpdatesFromAllLastTrades()
        {
            foreach (var populatedQuote in allFullyPopulatedQuotes)
            {
                ...
                foreach (var lastTrade in populatedQuote.RecentlyTraded)
                {
                    Assert.IsTrue(lastTrade.HasUpdates);
                }
        ...
        public void AllFullyPopulatedQuotes_Reset_SameAsEmptyQuotes()
        {
            ...
            for (int i = 0; i < allFullyPopulatedQuotes.Count; i++)
            {
        ...
        public void FullyPopulatedQuote_HasNoUpdatesCopyFrom_OnlyCopiesMinimalData()
        {
            foreach (var pqLevel3Quote in allFullyPopulatedQuotes)
            {
                ...
                foreach (var pvl in emptyQuote.BidBook)
                {
                    PQLevel2QuoteTests.AssertAreDefaultValues(pvl);
                }
                foreach (var pvl in emptyQuote.AskBook)
                {
                    PQLevel2QuoteTests.AssertAreDefaultValues(pvl);
                }
                if (emptyQuote.RecentlyTraded == null) continue;
                foreach (var lastTraded in emptyQuote.RecentlyTraded)
                {
                    AssertAreDefaultValues(lastTraded);
                }
            }
        }
        ...
        public void FullyPopulatedQuoteCloned_OneDifferenceAtATimeAreEquivalentExact_CorrectlyReturnsWhenDifferent()
        {
            foreach (var populatedL3Quote in allFullyPopulatedQuotes)
            {
                ...
                fullyPopulatedClone.SourceTickerQuoteInfo
                    = new PQSourceTickerQuoteInfo(populatedL3Quote.SourceTickerQuoteInfo);
                AssertAreEquivalentMeetsExpectedExactComparisonType(true, populatedL3Quote,
                    fullyPopulatedClone);
                AssertAreEquivalentMeetsExpectedExactComparisonType(false, populatedL3Quote,
                    fullyPopulatedClone);
            }
        }
            ...
            PQLevel3Quote original, PQLevel3Quote changingLevel3Quote)
        {
            ...
            {
                PQRecentlyTradedTests.AssertAreEquivalentMeetsExpectedExactComparisonType(exactComparison,
                    original.RecentlyTraded as PQRecentlyTraded, changingLevel3Quote.RecentlyTraded as PQRecentlyTraded, 
            ...
            PQLevel3Quote l3Q, uint expectedBooleanFlags = 3)
        {
            ...
            for (int i = 0; i < PQFieldKeys.SingleByteFieldIdMaxPossibleLastTrades; i++)
            {
                ...
                {
                    var expectedFlag = pqPaidGivenTrade.WasGiven ? PQFieldFlags.IsGivenFlag : (byte)0;
                    expectedFlag |= pqPaidGivenTrade.WasPaid ? PQFieldFlags.IsPaidFlag : (byte)0;
