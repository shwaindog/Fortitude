...
using FortitudeTests.FortitudeMarketsCore.Pricing.PQ.Quotes.SourceTickerInfo;
...
namespace FortitudeTests.FortitudeMarketsCore.Pricing.PQ.Quotes
{
    ...
    public class PQLevel0QuoteTests
    {
        ...
        private ISourceTickerQuoteInfo blankSourceTickerQuoteInfo;
        private PQLevel0Quote emptyQuote;
        private PQLevel0Quote fullyPopulatedPqLevel0Quote;
        private PQLevel0Quote newlyPopulatedPqLevel0Quote;
        ...
        public void SetUp()
        {
            ...
            blankSourceTickerQuoteInfo = new SourceTickerQuoteInfo(0, null, null);
        ...
        public void EmptyQuote_SourceTimeChanged_ExpectedPropertiesUpdatedDeltaUpdatesAffected()
        {
            ...
            PQFieldUpdate expectedSubHour = new PQFieldUpdate(PQFieldKeys.SourceSentSubHourTime, subHourComponent, 15);
        ...
        public void EmptyQuote_SyncStatusChanged_ExpectedPropertyUpdatedDeltaUpdatesAffected()
        {
            ...
            var sourceUpdates = emptyQuote.GetDeltaUpdateFields(testDateTime, UpdateStyle.Updates).ToList();
            Assert.AreEqual(1, sourceUpdates.Count);
            PQFieldUpdate expectedFieldUpdate = new PQFieldUpdate(PQFieldKeys.PQSyncStatus, (byte)expectedSyncStatus);
        ...
        public void EmptyQuote_SingPriceChanged_ExpectedPropertyUpdatedDeltaUpdatesAffected()
        {
            ...
            var sourceUpdates = emptyQuote.GetDeltaUpdateFields(testDateTime, UpdateStyle.Updates).ToList();
            Assert.AreEqual(1, sourceUpdates.Count);
            PQFieldUpdate expectedFieldUpdate = new PQFieldUpdate(PQFieldKeys.SinglePrice, expectedSinglePrice, 1);
            Assert.AreEqual(expectedFieldUpdate, sourceUpdates[0]);
        ...
        public void EmptyQuote_ReplayChanged_ExpectedPropertyUpdatedDeltaUpdatesAffected()
        {
            ...
            var sourceUpdates = emptyQuote.GetDeltaUpdateFields(testDateTime, UpdateStyle.Updates).ToList();
            Assert.AreEqual(1, sourceUpdates.Count);
            PQFieldUpdate expectedFieldUpdate = new PQFieldUpdate(PQFieldKeys.QuoteBooleanFlags, 1);
            Assert.AreEqual(expectedFieldUpdate, sourceUpdates[0]);
        ...
        public void PopulatedQuote_GetDeltaUpdatesUpdatePersistenceThenUpdateFieldNewQuote_CopiesAllFieldsToNewQuote()
        {
            ...
            var pqFieldUpdates = fullyPopulatedPqLevel0Quote.GetDeltaUpdateFields(
                new DateTime(2017, 11, 04, 16, 33, 59), UpdateStyle.Updates | UpdateStyle.Persistence).ToList();
            var newEmpty = new PQLevel0Quote(sourceTickerQuoteInfo);
        ...
        {
            var pqFieldUpdates = fullyPopulatedPqLevel0Quote.GetDeltaUpdateFields(
                new DateTime(2017, 11, 04, 16, 33, 59), UpdateStyle.Updates | UpdateStyle.Replay).ToList();
            var newEmpty = new PQLevel0Quote(sourceTickerQuoteInfo);
            foreach (var pqFieldUpdate in pqFieldUpdates)
            {
                newEmpty.UpdateField(pqFieldUpdate);
        ...
        public void FullyPopulatedQuote_HasNoUpdatesCopyFrom_OnlyCopiesMinimalData()
        {
            ...
            Assert.AreEqual(DateTimeConstants.UnixEpoch, emptyQuote.SourceTime);
            Assert.IsTrue(blankSourceTickerQuoteInfo.AreEquivalent(emptyQuote.SourceTickerQuoteInfo));
            ...
            PQLevel0Quote originalQuote, uint expectedBooleanFlags = 1)
        {
            ...
            byte flag = sourceTime.GetSubHourComponent().BreakLongToByteAndUint(out var value);
        ...
        internal class DummyPQLevel0Quote : IPQLevel0Quote
        {
            ...
            public ISyncLock Lock { get; } = null;
            public IPQLevel0Quote Previous { get; set; }
            public IPQLevel0Quote Next { get; set; }
            public virtual object Clone() { return null; }
            ILevel0Quote ILevel0Quote.Clone() { return (ILevel0Quote)Clone(); }
            ILevel0Quote ICloneable<ILevel0Quote>.Clone() { return (ILevel0Quote)Clone(); }
            IMutableLevel0Quote IMutableLevel0Quote.Clone() { return (IMutableLevel0Quote)Clone(); }
            IPQLevel0Quote IPQLevel0Quote.Clone() { return (IPQLevel0Quote)Clone(); }
            ...
            public int UpdateField(PQFieldUpdate updates) { return -1; }
            public bool UpdateFieldString(PQFieldStringUpdate updates) { return false; }
            ...
            public bool AreEquivalent(ILevel0Quote other, bool exactTypes = false)
            {
                return false;
            }
        }
