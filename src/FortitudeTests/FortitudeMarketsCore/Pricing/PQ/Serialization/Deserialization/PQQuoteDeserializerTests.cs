...
namespace FortitudeTests.FortitudeMarketsCore.Pricing.PQ.Serialization.Deserialization
{
    ...
    {
        private PQQuoteDeserializer<IPQLevel0Quote> pqLevel0QuoteDeserializer;
        private PQQuoteDeserializer<IPQLevel1Quote> pqLevel1QuoteDeserializer;
        private PQQuoteDeserializer<IPQLevel2Quote> pqLevel2QuoteDeserializer;
        private PQQuoteDeserializer<IPQLevel3Quote> pqLevel3QuoteDeserializer;
        private IList<IPQDeserializer<IPQLevel0Quote>> deserializers;
        ...
        private PQLevel0Quote expectedL0FullyInitializedQuote;
        private PQLevel1Quote expectedL1FullyInitializedQuote;
        private PQLevel2Quote expectedL2FullyInitializedQuote;
        private PQLevel3Quote expectedL3FullyInitializedQuote;
        private IList<IPQLevel0Quote> expectedQuotes;
        private ISourceTickerClientAndPublicationConfig sourceTickerQuoteInfo;

        private Mock<IPerfLogger> moqDispatchPerfLogger;
        private PQQuoteDeserializationSequencedTestDataBuilder quoteDeserializerSequencedTestDataBuilder;
        ...
        public void SetUp()
        {
            ...
            pqLevel0QuoteDeserializer = new PQQuoteDeserializer<IPQLevel0Quote>(sourceTickerQuoteInfo);
            pqLevel1QuoteDeserializer = new PQQuoteDeserializer<IPQLevel1Quote>(sourceTickerQuoteInfo);
            pqLevel2QuoteDeserializer = new PQQuoteDeserializer<IPQLevel2Quote>(sourceTickerQuoteInfo);
            pqLevel3QuoteDeserializer = new PQQuoteDeserializer<IPQLevel3Quote>(sourceTickerQuoteInfo);

            SetupDefaultState();

            SetupEventListeners();

            deserializers = new List<IPQDeserializer<IPQLevel0Quote>>
            {
                pqLevel0QuoteDeserializer, pqLevel1QuoteDeserializer,
                pqLevel2QuoteDeserializer, pqLevel3QuoteDeserializer
            };
        ...
        [TestMethod]
        public void FreshSerializerMissedUpdates_DeserializeManyUnexpectedSeqId_StaysUnsync()
        {
        ...
        public void TimedOutDeserializer_HasTimedOutAndNeedsSnapshot_ReturnsFalseNeedsSnapshot()
        {
            ...
            AssertDeserializerHasTimedOutAndNeedsSnapshotIs(PQQuoteDeserializationSequencedTestDataBuilder
                .ClientReceivedTimestamp(PQQuoteDeserializationSequencedTestDataBuilder.TimeOffsetForSequenceId(21)), false);
            AssertDeserializerHasTimedOutAndNeedsSnapshotIs(PQQuoteDeserializationSequencedTestDataBuilder
                .ClientReceivedTimestamp(PQQuoteDeserializationSequencedTestDataBuilder.TimeOffsetForSequenceId(99)), false);
        }
        ...
        public void OutOfSyncDeserializer_RequestSnapshotReceivesUpdatesUpToBuffer_GoesInSyncPublishesLatestUpdate()
        {
            ...
            SendsSequenceIdFromTo(2, PQQuoteDeserializer<IPQLevel0Quote>.MaxBufferedUpdates, false);

            AssertQuotesAreInSync(false);
            AssertPublishCountIs(1);

            var snapshotSequence2 = quoteDeserializerSequencedTestDataBuilder.BuildQuotesStartingAt(1, 1, new List<int>()).First();
            SetupMockPublishQuoteIsExpected();

            quoteDeserializerSequencedTestDataBuilder.BuildQuotesStartingAt(PQQuoteDeserializer<IPQLevel0Quote>
        ...
        public void OutOfSyncDeserializer_RequestSnapshotReceivesUpdatesMoreThanBuffer_PublishesNothing()
        {
            ...
            SendsSequenceIdFromTo(2, PQQuoteDeserializer<IPQLevel0Quote>.MaxBufferedUpdates+1, false);
            ...
            moqL0QObserver.Setup(o => o.OnNext(pqLevel0QuoteDeserializer.PublishedQuote))
                .Callback<IPQLevel0Quote>(
                    pq =>
                    {
                        Assert.Fail("Should not publish anything");
                    }
                ).Verifiable();
            moqL1QObserver.Setup(o => o.OnNext(pqLevel1QuoteDeserializer.PublishedQuote))
                .Callback<IPQLevel1Quote>(
        ...
        {
            var quoteBatches = quoteDeserializerSequencedTestDataBuilder.BuildQuotesStartingAt(startId, batchSize, new List<int>());

            ResetObserverMockExpectations();
            foreach (var quoteBatch in quoteBatches.Take(1))
            {
                CallDeserializer(quoteBatch);
            }
            l0QuoteSame = false;
            l1QuoteSame = false;
            l2QuoteSame = false;
            ...
            foreach (var quoteBatch in quoteBatches.Skip(1))
            {
                CallDeserializer(quoteBatch);
            }
        ...
        {
            for (int i = 0; i < deserializers.Count; i++)
            {
            ...
            IList<DispatchContext> deserializeInputList)
        {
            for (int i = 0; i < deserializers.Count; i++)
            {
                deserializers[i].Deserialize(deserializeInputList[i]);
            }
        }
        ...
        private void SetupPQLevelQuotes()
        {
            ...
            {
                expectedL0FullyInitializedQuote,
                expectedL1FullyInitializedQuote,
                expectedL2FullyInitializedQuote,
