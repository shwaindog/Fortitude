...
public class PQLevel0QuoteTests
{
    ...
    public void SetUp()
    {
        ...
        sourceTickerQuoteInfo =
            new PQSourceTickerQuoteInfo
               ...
               , 20, 0.0000001m, 30000m, 50000000m, 1000m, 1
        ...
        fullyPopulatedPqLevel0Quote = new PQLevel0Quote(new PQSourceTickerQuoteInfo(sourceTickerQuoteInfo));
        emptyQuote                  = new PQLevel0Quote(new PQSourceTickerQuoteInfo(sourceTickerQuoteInfo)) { HasUpdates = false };
    ...
    public void EmptyQuote_SyncStatusChanged_ExpectedPropertyUpdatedDeltaUpdatesAffected()
    {
        ...
        var expectedSyncStatus = PriceSyncStatus.Good;
        emptyQuote.PQPriceSyncStatus = expectedSyncStatus;
        Assert.IsTrue(emptyQuote.IsSyncStatusUpdated);
        Assert.IsTrue(emptyQuote.HasUpdates);
        Assert.AreEqual(expectedSyncStatus, emptyQuote.PQPriceSyncStatus);
        var sourceUpdates = emptyQuote.GetDeltaUpdateFields(testDateTime, StorageFlags.Update).ToList();
        Assert.AreEqual(1, sourceUpdates.Count);
        ...
        var newEmpty = new PQLevel0Quote(sourceTickerQuoteInfo);
        newEmpty.UpdateField(sourceUpdates[0]);
        Assert.AreEqual(expectedSyncStatus, newEmpty.PQPriceSyncStatus);
    ...
    public void EmptyQuote_SingPriceChanged_ExpectedPropertyUpdatedDeltaUpdatesAffected()
    {
        ...
        var priceScale          = sourceTickerQuoteInfo.PriceScalingPrecision;
        ...
        var sourceUpdates = emptyQuote.GetDeltaUpdateFields(testDateTime, StorageFlags.Update).ToList();
        Assert.AreEqual(1, sourceUpdates.Count);
        var expectedFieldUpdate = new PQFieldUpdate(PQFieldKeys.SinglePrice, PQScaling.Scale(expectedSinglePrice, priceScale), priceScale);
        Assert.AreEqual(expectedFieldUpdate, sourceUpdates[0]);
    ...
    public void PopulatedQuoteWithAllUpdates_GetDeltaUpdateFieldsAsUpdate_ReturnsAllLevel0Fields()
    {
        ...
        AssertContainsAllLevel0Fields(sourceTickerQuoteInfo, pqFieldUpdates, fullyPopulatedPqLevel0Quote);
    }
        ...
        (bool exactComparison, PQLevel0Quote original, PQLevel0Quote changingLevel0Quote)
    {
        ...
        changingLevel0Quote.PQPriceSyncStatus = PriceSyncStatus.FeedDown;
        Assert.AreEqual(!exactComparison, original.AreEquivalent(changingLevel0Quote, exactComparison));
        changingLevel0Quote.PQPriceSyncStatus = original.PQPriceSyncStatus;
        Assert.IsTrue(changingLevel0Quote.AreEquivalent(original, exactComparison));
    ...
    (IPQPriceVolumePublicationPrecisionSettings precisionSettings, IList<PQFieldUpdate> checkFieldUpdates, PQLevel0Quote originalQuote
      , PQBooleanValues expectedBooleanFlags = PQBooleanValues.IsReplayUpdatedFlag | PQBooleanValues.IsReplaySetFlag)
    {
        ...
        Assert.AreEqual(new PQFieldUpdate(PQFieldKeys.PQSyncStatus, (uint)originalQuote.PQPriceSyncStatus),
                        ExtractFieldUpdateWithId(checkFieldUpdates, PQFieldKeys.PQSyncStatus));
        Assert.AreEqual(new PQFieldUpdate(PQFieldKeys.SinglePrice, PQScaling.Scale(originalQuote.SinglePrice, priceScale), priceScale),
    ...
    internal class DummyPQLevel0Quote : ReusableObject<ILevel0Quote>, IPQLevel0Quote, IStoreState<DummyPQLevel0Quote>
    {
        ...
        public PriceSyncStatus PQPriceSyncStatus { get; set; }
