...
namespace FortitudeTests.FortitudeMarketsCore.Pricing.LastTraded
{
    ...
    {
        private RecentlyTraded simpleRecentlyTradedFullyPopulatedQuote;
        private RecentlyTraded paidGivenVolumeRecentlyTradedFullyPopulatedQuote;
        private RecentlyTraded traderPaidGivenVolumeRecentlyTradedFullyPopulatedQuote;

        private IList<RecentlyTraded> allFullyPopulatedRecentlyTraded;

        private IList<IMutableLastTrade> simpleEntries;
        private IList<IMutableLastPaidGivenTrade> lastPaidGivenEntries;
        private IList<IMutableLastTraderPaidGivenTrade> lastTraderPaidGivenEntries;

        private List<IReadOnlyList<ILastTrade>> allPopulatedEntries;
        ...
        {
            for (int i = 0; i < allFullyPopulatedRecentlyTraded.Count; i++)
            {
        ...
        public void PopulatedRecentlyTraded_AccessIndexerVariousInterfaces_GetsAndSetsLayerRemovesLastEntryIfNull()
        {
            foreach (var populatedOrderBook in allFullyPopulatedRecentlyTraded)
            {
                for (int i = 0; i < MaxNumberOfEntries; i++)
                {
                    ...
                    var clonedLayer = (IMutableLastTrade)layer.Clone();
        ...
        public void PopulatedRecentlyTraded_Count_UpdatesWhenPricesChanged()
        {
            foreach (var populatedRecentlyTraded in allFullyPopulatedRecentlyTraded)
            {
                for (int i = MaxNumberOfEntries - 1; i >= 0; i--)
                {
        ...
        public void PopulatedRecentlyTraded_Reset_ResetsAllEntries()
        {
            ...
            {
                Assert.AreEqual(MaxNumberOfEntries, populatedRecentlyTraded.Count);
                foreach (var pvl in populatedRecentlyTraded)
                {
        ...
        public void FullyPopulatedPvlVariousInterfaces_GetEnumerator_OnlyGetsNonEmptyEntries()
        {
            ...
            Assert.AreEqual(0, ((IEnumerable)rt).OfType<ILastTrade>().Count());
        }
        ...
        private static RecentlyTraded CreateNewEmpty(RecentlyTraded populatedOrderBook)
        {
            ...
            for (int i = 0; i < MaxNumberOfEntries; i++)
            {
                clonedEmptyEntries.Add(cloneGensis.Clone());
            }
        ...
        {
            if (copySourceType == typeof(LastTrade))
            {
                return true;
            }
            ...
            IMutableLevel3Quote originalQuote = null, IMutableLevel3Quote changingQuote = null)
        {
            if (original == null)
            {
                Assert.IsNull(changingRecentlyTraded);
                return;
            }
            ...
            for (int i = 0; i < original.Count; i++)
            {
                ...
                LastPaidGivenTradeTests.AssertAreEquivalentMeetsExpectedExactComparisonType(
                    exactComparison, originalEntry as IMutableLastPaidGivenTrade, 
                    changingEntry as IMutableLastPaidGivenTrade, original,
                    changingRecentlyTraded, originalQuote, changingQuote);
                LastTraderPaidGivenTradeTests.AssertAreEquivalentMeetsExpectedExactComparisonType(
                    exactComparison, originalEntry as IMutableLastTraderPaidGivenTrade,
                    changingEntry as IMutableLastTraderPaidGivenTrade, original,
                    changingRecentlyTraded, originalQuote, changingQuote);
            }
        }
    }
