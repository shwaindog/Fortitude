...
public class ActionTimerCallBackRunInfoTests
{
    ...
    private ActionStateTimerCallBackRunInfo<string> timerCallBackRunInfo = null!;

    private Action<string?> waitCallback = null!;

    [TestInitialize]
    public void SetUp()
    {
        callbackCounter = 0;
        waitCallback = stringState =>
        {
    ...
    private ActionStateTimerCallBackRunInfo<string> CreateTimerCallBackRunInfo(DateTime firstScheduledTime) =>
        new()
        {
            Action = waitCallback, FirstScheduledTime = firstScheduledTime, LastRunTime = DateTime.MinValue
