...
namespace FortitudeTests.FortitudeCommon.DataStructures.Memory
{
    ...
    public class GarbageAndLockFreePooledFactoryTests
    {
        ...
        {
            public LendTimes(DateTime borrowTime) : this()
            {
        ...
        private class LendItemTrackingTimes
        {
            ...
            {
                int slotNum = Interlocked.Increment(ref nextSlotCounter);
        ...
        private readonly Random randomNumberGen = new Random();
        ...
        public void SetUp()
        {
            ...
            for (int i = 0; i < itemTracking.Length; i++)
            {
                itemTracking[i] = new LendItemTrackingTimes();
            }
        }
        ...
        {
            GarbageAndLockFreePooledFactory<String> pool = new GarbageAndLockFreePooledFactory<string>(
            ...
            foreach (var queuedString in pool)
            {
                Assert.IsTrue(allStrings.Contains(queuedString));
            }
            ...
            foreach (var queuedString in pool)
            {
                Assert.AreNotSame(fourthString, queuedString);
            }
            ...
            foreach (var queuedString in pool)
            {
                Assert.AreNotSame(sixthString, queuedString);
            }
            ...
            foreach (var queuedString in pool)
            {
                Assert.AreNotSame(firstString, queuedString);
            }
        }
        ...
        public void MultipleReadersAndWritersEnqueingNoElementIsDoubleBooked()
        {
            ...
            for (int i = 0; i < itemTracking.Length; i++)
            {
                ...
                for (int j = 0; j < maxNumberOfSlots; j++)
                {
                    ...
                    for (int k = j+1; k < maxNumberOfSlots && k < j + MaxNumberThreads*2; k++)
                    {
        ...
        private void RunExercise()
        {
            ...
            try
            {
                for (i = 0; i < BorrowReturnIterations; i++)
                {
                    ...
                    DateTime returnTime = TimeContext.LocalTimeNow;
