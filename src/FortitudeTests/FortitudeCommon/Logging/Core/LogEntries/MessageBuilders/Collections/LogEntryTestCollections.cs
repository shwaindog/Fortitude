...
public class LogEntryTestCollections
{
    ...
    {
        { 0, Math.PI }, { 1, Math.E }, { 2, -Math.PI*100.0 }, { 3, Math.E*10_000.0*Math.PI }, { 4, Math.PI*97.0/Math.E }, { 5, Math.E*Math.PI },
        { 6, Math.PI*1_000_000.0 + Math.E*100 }, { 7, Math.PI/999_983.0 } , { 8, Math.E/999_983.0 }, { 10, 0 }, { 11, -1.0 }
    };
    ...
    public static readonly Dictionary<TimeSpan, decimal[]?> TimeSpanToDecimalArrayMap = new()
    {
        ...
        { TimeSpan.Zero, [ new decimal(Math.E*10_000.0*Math.PI), new decimal(long.MinValue) + new decimal(Math.PI)] }, 
        { TimeSpan.FromDays(1), [] }, 
        { TimeSpan.FromHours(12).Add(TimeSpan.FromMinutes(42)), null }, 
        { TimeSpan.FromHours(16).Add(TimeSpan.FromMinutes(42)), [ decimal.Zero] }, 
    };
    ...
    public static readonly Dictionary<BigInteger, List<decimal?>?> BigIntToNullableDecimalListMap = new()
    {
        ...
        { new BigInteger(ulong.MaxValue) * new BigInteger(3), [ new decimal(Math.E*10_000.0*Math.PI), new decimal(long.MinValue) + new decimal(Math.PI)] }, 
        { new BigInteger(1), [] }, 
    ...
    public static readonly List<KeyValuePair<BigInteger, List<decimal?>?>> BigIntToNullableDecimalListMapList  = BigIntToNullableDecimalListMap.ToList();
    ...
    public static readonly Dictionary<FLogExampleConfig, FLogAppConfig?> StructToStyledToStringObjMap = new()
    {
      ...
      , { FLogConfigExamples.AsyncDblBufferedColoredConsoleExample, FLogConfigExamples.AsyncDblBufferedColoredConsoleExample.LoadExampleToAppConfig()}
