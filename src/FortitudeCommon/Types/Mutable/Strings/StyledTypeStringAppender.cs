...
public interface IStyledTypeStringAppender : IReusableObject<IStyledTypeStringAppender>
{
    ...
    void ClearSetStyle(StringBuildingStyle stringStyle, int indentLevel = 0, IgnoreWriteFlags ignoreFlags = IgnoreWriteFlags.None);
    ...
    IStyledTypeStringAppender AddKeyValues<TKey, TValue>(IEnumerable<KeyValuePair<TKey, TValue>>? value);
...
public class WrappingStyledTypeStringAppender : ReusableObject<IStyledTypeStringAppender>, IStyledTypeStringAppender
{
    ...
    protected TypeAppendSettings CurrentType;

    protected Stack<TypeAppendSettings>? SettingsStack;
    ...
    public int IndentLevel => CurrentType.IndentLvl;

    public IgnoreWriteFlags IgnoreWriteFlags => CurrentType.IgnoreWriteFlags;
    ...
    public void ClearSetStyle(StringBuildingStyle stringStyle, int indentLevel = 0, IgnoreWriteFlags ignoreWrite = IgnoreWriteFlags.None)
    {
        BuildStyle           = stringStyle;
        CurrentType = new TypeAppendSettings((ushort)indentLevel, ignoreWrite);
        Sb.Clear();
    }

    public IStyledTypeStringAppender DecrementIndent()
    {
        CurrentType.IndentLvl--;
        return this;
    }
    ...
    {
        if (!CurrentType.IgnoreWriteFlags.HasTypeNameFlag())
        {
            if (BuildStyle.IsLogCompact()) Sb.Append(value);
        }
        else
        {
            CurrentType.IgnoreWriteFlags ^= IgnoreWriteFlags.TypeName;
        }
        return this;
    }
    ...
    {
        if (!CurrentType.IgnoreWriteFlags.HasTypeStartFlag())
        {
            Sb.Append("{");
            CurrentType.IndentLvl++;
        }
        ...
        {
            CurrentType.IgnoreWriteFlags ^= IgnoreWriteFlags.TypeStart;
        }
        PushCurrentSettings();
        return this;
    }
    ...
    {
        PopCurrentSettings();
        if (!CurrentType.IgnoreWriteFlags.HasTypeEndFlag())
        {
            ...
            Sb.Append("}");
            CurrentType.IndentLvl--;
            if (CurrentType.IndentLvl > 0 && BuildStyle.IsPretty())
            {
                Sb.Append(",\n");
    ...
    protected void PushCurrentSettings(IgnoreWriteFlags newIgnoreFlags = IgnoreWriteFlags.None)
    {
        SettingsStack ??= new Stack<TypeAppendSettings>();
        SettingsStack.Push(CurrentType);
        CurrentType.IgnoreWriteFlags = newIgnoreFlags;
    }

    protected void PopCurrentSettings()
    {
        SettingsStack ??= new Stack<TypeAppendSettings>();
        if (SettingsStack.TryPop(out var result))
        {
            CurrentType = result;
        }
    }

    public IStyledTypeStringAppender AddBaseFieldsEnd()
    {
        PushCurrentSettings(IgnoreWriteFlags.TypeStart | IgnoreWriteFlags.TypeName | IgnoreWriteFlags.TypeEnd);

        return this;
    }

    public IStyledTypeStringAppender AddBaseFieldsStart()
    {
        PopCurrentSettings();
    ...
    {
        Sb.AddIndents(Indent,  CurrentType.IndentLvl - 1);
        return this;
    }
    ...
    internal void StartCollection(string fieldName)
    {
        ...
        {
            CurrentType.IndentLvl++;
            Sb.Append("\n").AddIndents(Indent, CurrentType.IndentLvl);
        }
    }
    ...
    public IStyledTypeStringAppender AddKeyValues<TKey, TValue>(IEnumerable<KeyValuePair<TKey, TValue>>? value)
    {
        if (value != null)
            foreach (var kvp in value)
            {
...
public class StyledTypeStringAppender : WrappingStyledTypeStringAppender
{
    ...
    public StyledTypeStringAppender(IStyledTypeStringAppender toClone)
    {
        ...
        CurrentType  = new TypeAppendSettings((ushort)toClone.IndentLevel, toClone.IgnoreWriteFlags);
