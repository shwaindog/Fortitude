...
public partial class SelectTypeField<TExt> where TExt : TypeMolder
{
    ...
    public TExt WhenNonDefaultRevealAs<TCloaked, TCloakedBase>(ReadOnlySpan<char> fieldName, TCloaked? value, PalantírReveal<TCloakedBase> palantírReveal
    ...
    public TExt WhenNonDefaultReveal<TCloakedStruct>(ReadOnlySpan<char> fieldName, TCloakedStruct? value, PalantírReveal<TCloakedStruct> palantírReveal, TCloakedStruct? defaultValue = null)
        where TCloakedStruct : struct =>
        !stb.SkipFields && !Equals(value, defaultValue)
            ? AlwaysReveal(fieldName, value, palantírReveal)
            : stb.StyleTypeBuilder;

    public TExt WhenNonDefaultRevealAs<TCloakedStruct>(ReadOnlySpan<char> fieldName, TCloakedStruct? value, PalantírReveal<TCloakedStruct> palantírReveal, TCloakedStruct? defaultValue = null
      , FieldContentHandling flags = FieldContentHandling.DefaultForValueType) where TCloakedStruct : struct =>
        !stb.SkipFields && !Equals(value, defaultValue)
            ? AlwaysRevealAs(fieldName, value, palantírReveal, flags)
            : stb.StyleTypeBuilder;

    public TExt WhenNonDefaultReveal<TBearer>(ReadOnlySpan<char> fieldName, TBearer? value, TBearer? defaultValue = default(TBearer)) where TBearer : IStringBearer =>
        !stb.SkipFields && !Equals(value, defaultValue)
    ...
    public TExt WhenNonDefaultReveal<TBearerStruct>(ReadOnlySpan<char> fieldName, TBearerStruct? value, TBearerStruct? defaultValue = null) where TBearerStruct : struct, IStringBearer =>
        !stb.SkipFields && !Equals(value, defaultValue)
            ? AlwaysReveal(fieldName, value)
            : stb.StyleTypeBuilder;
    ...
    public TExt WhenNonDefaultAddCharSeqAs<TCharSeq>(ReadOnlySpan<char> fieldName, TCharSeq value, int startIndex, int count = int.MaxValue, string defaultValue = ""
      , FieldContentHandling flags = FieldContentHandling.DefaultForValueType
    , [StringSyntax(StringSyntaxAttribute.CompositeFormat)] string? formatString = null) where TCharSeq : ICharSequence =>
        !stb.SkipFields && !(value == null! || value.Equals(defaultValue))
    ...
    public TExt WhenNonDefaultAdd(ReadOnlySpan<char> fieldName, StringBuilder value, int startIndex, int count = int.MaxValue, string defaultValue = ""
      , [StringSyntax(StringSyntaxAttribute.CompositeFormat)] string? formatString = null) =>
        !stb.SkipFields && !(value == null! || value.Equals(defaultValue))
            ? AlwaysAdd(fieldName, value, startIndex, count, formatString)
            : stb.StyleTypeBuilder;

    public TExt WhenNonDefaultAddAs(ReadOnlySpan<char> fieldName, StringBuilder value, int startIndex, int count = int.MaxValue, string defaultValue = ""
      , FieldContentHandling flags = FieldContentHandling.DefaultForValueType
      , [StringSyntax(StringSyntaxAttribute.CompositeFormat)] string? formatString = null) =>
        !stb.SkipFields && !(value == null! || value.Equals(defaultValue))
    ...
    public TExt WhenNonDefaultAddMatch<T>(ReadOnlySpan<char> fieldName, T? value, T? defaultValue = default(T)
      , [StringSyntax(StringSyntaxAttribute.CompositeFormat)] string? formatString = null) =>
        !stb.SkipFields && !(value == null! || value.Equals(defaultValue))
