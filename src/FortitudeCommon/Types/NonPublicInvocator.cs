...
namespace FortitudeCommon.Types
{
    public class NonPublicInvocator
    {
        ...
        {
            BindingFlags flags = BindingFlags.NonPublic | (includePubWithNonPubSetters 
                ? BindingFlags.Public 
        ...
        {
            return GetInstanceProperty(objInstance.GetType(), objInstance, propertyName);
        }
        ...
        public static object GetInstanceProperty(Type t, object objInstance, string propertyName)
        {
            ...
            if (propInfo == null)
            {
                if (t != typeof(object))
                {
                    return GetInstanceProperty(t.BaseType, objInstance, propertyName);
                }
            }
        ...
        {
            return GetInstanceField<T>(objInstance.GetType(), objInstance, fieldName);
        }
        ...
        public static T GetInstanceField<T>(Type t, object objInstance, string fieldName)
        {
            ...
            if (fieldInfo == null)
            {
                if (t != typeof(object))
                {
                    return GetInstanceField<T>(t.BaseType, objInstance, fieldName);
                }
            }
            else 
            {
            ...
            U value)
        {
            ...
            {
                throw new Exception($"No property found in type {objInstance.GetType()}");
            }
        ...
        public static object GetInstance(Type t, params object[] constructorParams)
        {
            ...
            {
                if (typesArrayAsString.Length > 1)
                {
                    typesArrayAsString += ", ";
                }
        ...
        public static T GetInstance<T>(params object[] constructorParams)
        {
            return (T) GetInstance(typeof(T), constructorParams);
        }
        ...
        private static T RunMethod<T>(Type t, object objInstance, string strMethod, object[] aobjParams, 
            BindingFlags flags)
        {
            ...
            Type[] paramTypes = new Type[aobjParams.Length];
            for (int i = 0; i < aobjParams.Length; i++)
            {
                paramTypes[i] = aobjParams[i].GetType();
            }
            MethodInfo m = null;
            Type originalType = t;
            while ((t != null && t != typeof(object)) && m == null)
            {
                m = t.GetMethod(strMethod, flags, null, paramTypes, null);
                t = t.BaseType;
            }
            if (m == null)
            {
                throw new ArgumentException("There is no method '" + strMethod + "' for type '" + originalType + 
                    "' or its base methods.");
            }
            ...
            BindingFlags flags)
        {
            ...
            for (var i = 0; i < aobjParams.Length; i++)
            {
                paramsAddedInstance[i + 1] = aobjParams[i];
            }
