...
public class NonPublicInvocator
{
    ...
    public static T RunInstanceMethodBaseVersion<T>
    (object objInstance, string strMethod,
    ...
    public static void SetInstanceProperty<T>
    (object objInstance, string propertyName, T value,
    ...
    public static void SetStaticProperty<T>
    (Type? t, string propertyName, T value,
        bool includePubWithNonPubSetters = false)
    {
        var flags = BindingFlags.NonPublic | (includePubWithNonPubSetters ? BindingFlags.Public : BindingFlags.NonPublic)
    ...
    public static T GetInstanceField<T>(object objInstance, string fieldName) => GetInstanceField<T>(objInstance.GetType(), objInstance, fieldName)!;
    ...
    public static void SetAutoPropertyInstanceField<T, U>
    (object objInstance, Expression<Func<T, U>> expression,
        U value)
    {
    ...
    public static void SetInstanceField<T>
    (object objInstance, string fieldName, T value,
        bool includePubWithNonPubSetters = false)
    {
        var flags
    ...
    public static FieldInfo SetInstanceFieldFieldInfo<TInstance>
    (TInstance objInstance, string fieldName,
        bool includePubWithNonPubSetters = false)
    {
        var flags
            = BindingFlags.NonPublic | (includePubWithNonPubSetters ? BindingFlags.Public : BindingFlags.NonPublic)
        ...
        if (fieldInfo == null) throw new ApplicationException($"Requested field '{fieldName}' could not be found on {objInstance!.GetType().Name}");

        return fieldInfo;
    }

    public static void SetStaticField<T>
    (Type type, string fieldName, T value,
        bool includePubWithNonPubSetters = false)
    {
        var flags = BindingFlags.NonPublic |
        ...
        if (fieldInfo == null) throw new ApplicationException($"Requested field '{fieldName}' could not be found on {type.Name}");
    ...
    public static object GetInstance(Type t, params object[] constructorParams)
    {
        ...
        throw new NotImplementedException
            ("No such constructor can be found for " + t.FullName + " taking parameters " + typesArrayAsString + "\n" +
    ...
    private static Func<T, TU, TV> GetInstanceMethodFunc<T, TU, TV>
    (object objInstance, string strMethod
      , BindingFlags flags)
    {
        Type t;
    ...
    private static T RunMethod<T>
    (Type t, object? objInstance, string strMethod, object[] objParams,
        BindingFlags flags)
    {
    ...
    private static Action<T> GetInstanceMethodAction<T>
    (Type t, object? objInstance, string strMethod,
        BindingFlags flags)
    {
        if (objInstance == null) objInstance = t;
        ...
        if (m == null)
            throw new ArgumentException
                ("There is no method '" + strMethod + "' for type '" + currentType + "' or its base methods.");
    ...
    public static Action<T> GetStaticMethodAction<T>
        (Type t, string strMethod, BindingFlags flags = BindingFlags.NonPublic | BindingFlags.Static)
    {
        var paramTypes = new Type[1];
        paramTypes[0] = typeof(T);
        ...
        var callBackDelegate = m.CreateDelegate<Action<T>>(null);
        return callBackDelegate;
    }

    public static Func<T> GetStaticMethodFunc<T>
        (Type t, string strMethod, BindingFlags flags = BindingFlags.NonPublic | BindingFlags.Static)
    {
        MethodInfo? m = null;
        ...
        while (currentType != null && currentType != typeof(object) && m == null)
        {
            m = currentType.GetMethod(strMethod, flags, null, Array.Empty<Type>(), null);
        ...
        var callBackDelegate = m.CreateDelegate<Func<T>>(null);
        return callBackDelegate;
    }

    private static T RunMethodNoVirtual<T>(Type t, object? objInstance, string strMethod, object[] aobjParams, BindingFlags flags)
    {
        if (objInstance == null) objInstance = t;
    ...
    private static object? InvokeNonVirtual(MethodInfo method, object[] args)
    {
        ...
        for (var i = 0; i < paramTypes.Count; i++) il.Emit(OpCodes.Ldarg, i); // load all args
