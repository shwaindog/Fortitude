...
public class ReusableObjectItemSerde<T> : IDeserializer<T>, ISerializer<T> where T : class, IReusableObject, new()
{
    ...
    public void Serialize(T obj, ISerdeContext writeContext)
    {
        ...
        if (writeContext is IPooledRecyclableContext<T> existingItemContext) existingItemContext.LastItem = obj;
        else throw new ArgumentException("Expected readContext to be a IPooledRecyclableContext");
    }
}
