...
// Adapted System.Threading.Tasks.Sources.ManualResetValueTaskSourceCore<TResult> full credit to Microsoft Team
// Adapted to support recycling of SendOrPostCallback via a recyclable object
...
public struct RecyclingValueTaskSource<TResult>
{
    /// <summary>
    ///     The callback to invoke when the operation completes if <see cref="OnCompleted" /> was called before the operation
    ///     completed,
    ///     or <see cref="RecyclingValueTaskSourceCompanion.s_sentinel" /> if the operation completed before a callback was
    ///     supplied,
    ...
    ///     A scheduler (TaskScheduler or SynchronizationContext) if one was captured and needs to be used for callback
    ///     scheduling.
    ...
    {
        if (token != _version || !_completed || _error is not null) ThrowForFailedGetResult();
    ...
    private void ThrowForFailedGetResult()
    {
        _error?.Throw();
        throw
            new InvalidOperationException(); // not using ThrowHelper.ThrowInvalidOperationException so that the JIT sees ThrowForFailedGetResult as always throwing
    }
    ...
    {
        if (continuation is null) throw new ArgumentException("continuation is null");
        ValidateToken(token);

        if ((flags & ValueTaskSourceOnCompletedFlags.FlowExecutionContext) != 0) _capturedContext = ExecutionContext.Capture();
        ...
        if (!ReferenceEquals(storedContinuation, RecyclingValueTaskSourceCompanion.s_sentinel)) throw new InvalidOperationException();
        ...
        switch (capturedContext)
        {
            ...
            default:
                RecyclingValueTaskSourceCompanion.ScheduleCapturedContext(capturedContext, continuation, state, Recycler);
                break;
        }
    }
    ...
    private void SignalCompletion()
    {
        ...
        Action<object?>? continuation =
            ...
            Interlocked.CompareExchange(ref _continuation, RecyclingValueTaskSourceCompanion.s_sentinel, null);
        ...
        if (continuation is not null)
        {
            ...
            if (context is null)
            {
                if (_runContinuationsAsynchronously)
                    ThreadPool.UnsafeQueueUserWorkItem(continuation, _continuationState, true);
                else
                    continuation(_continuationState);
            }
            else if (context is ExecutionContext or CapturedSchedulerAndExecutionContext)
            {
                RecyclingValueTaskSourceCompanion.InvokeContinuationWithContext(context, continuation, _continuationState
                                                                              , _runContinuationsAsynchronously, Recycler);
            }
            else
            {
                Debug.Assert(context is TaskScheduler or SynchronizationContext, $"context is {context}");
                RecyclingValueTaskSourceCompanion.ScheduleCapturedContext(context, continuation, _continuationState, Recycler);
            }
        }
    }
}
...
internal static class RecyclingValueTaskSourceCompanion // separated out of generic to avoid unnecessary duplication
{
    internal static readonly Action<object?> s_sentinel = CompletionSentinel;

    private static Func<ExecutionContext?>   callCaptureExecutionContextForRestore;
    private static Action<ExecutionContext?> callRestoreExecutionInternal;

    static RecyclingValueTaskSourceCompanion()
    {
        callCaptureExecutionContextForRestore
            = NonPublicInvocator.GetStaticMethodFunc<ExecutionContext?>(typeof(ExecutionContext), "CaptureForRestore");
        callRestoreExecutionInternal = NonPublicInvocator.GetStaticMethodAction<ExecutionContext?>(typeof(ExecutionContext), "RestoreInternal");
    }
    ...
    internal static void ScheduleCapturedContext(object context, Action<object?> continuation, object? state, IRecycler recycler)
    {
        Debug.Assert(
        ...
        switch (context)
        {
            case SynchronizationContext sc:
                ScheduleSynchronizationContext(sc, continuation, state, recycler);
                break;
            ...
            default:
                ...
                {
                    ScheduleSynchronizationContext(ccsc, continuation, state, recycler);
                }
                else
                {
        ...
        static void ScheduleSynchronizationContext(SynchronizationContext sc, Action<object?> continuation, object? state, IRecycler recycler)
        {
            var reusableSendOrPostCallback = recycler.Borrow<ActionToSendOrPostRecyclable>();
            reusableSendOrPostCallback.ActionToInvoke = continuation;
            sc.Post(reusableSendOrPostCallback.SendOrPostCallback, state);
        }
    ...
    internal static void InvokeContinuationWithContext
        (object capturedContext, Action<object?> continuation, object? continuationState, bool runContinuationsAsynchronously, IRecycler recycler)
    {
        ...
        var currentContext = callCaptureExecutionContextForRestore();

        if (capturedContext is ExecutionContext ec)
        {
            callRestoreExecutionInternal(ec); // Restore the captured ExecutionContext before executing anything.
            if (runContinuationsAsynchronously)
            {
                try
                {
                    ThreadPool.QueueUserWorkItem(continuation, continuationState, true);
                }
                finally
                {
                    callRestoreExecutionInternal(currentContext); // Restore the current ExecutionContext.
                }
            }
            else
            {
                ...
                finally
                {
                    ...
                    callRestoreExecutionInternal(currentContext);
                }
        ...
        else
        {
            ...
            try
            {
                ScheduleCapturedContext(capturedContext, continuation, continuationState, recycler);
            }
            finally
            {
                callRestoreExecutionInternal(currentContext); // Restore the current ExecutionContext.
