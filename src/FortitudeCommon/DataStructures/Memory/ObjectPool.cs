...
namespace FortitudeCommon.DataStructures.Memory
{
    public class ObjectPool<T> where T : class
    {
        ...
        internal DoublyLinkedList<DoublyLinkedListWrapperNode<T>> AvailablePool =
            new DoublyLinkedList<DoublyLinkedListWrapperNode<T>>();

        private SpinLock poolSpinLock = new SpinLock();
        private SpinLock returnSpinLock = new SpinLock();

        internal DoublyLinkedList<DoublyLinkedListWrapperNode<T>> ReturnsPool =
            new DoublyLinkedList<DoublyLinkedListWrapperNode<T>>();
        ...
        public T Borrow()
        {
            ...
            for (var count = 0; count < 10; count++)
            {
                ...
                try
                {
                    ...
                    {
                        AvailablePool.Remove(requestedDoublyLinkedListWrapper);
                    }
                }
                finally
                {
                ...
                if (requestedDoublyLinkedListWrapper != null)
                {
                    ...
                    requestedDoublyLinkedListWrapper.Payload = null;
                    try
                    {
                        returnSpinLock.Enter(ref returnLocked);
        ...
        public void Return(T notUsed)
        {
            ...
            for (var count = 0; count < 10; count++)
            {
                ...
                try
                {
                    ...
                    {
                        ReturnsPool.Remove(requestedWrapper);
                    }
                }
                finally
                {
