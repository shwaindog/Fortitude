...
namespace FortitudeCommon.Monitoring.Logging.Diagnostics.CallStats
{
    public class CallStatsLogger : ICallStatsLogger
    {
        ...
        private static readonly ConcurrentDictionary<string, CallStatsLogger> InstanceTracker =
            new ConcurrentDictionary<string, CallStatsLogger>();
        ...
        private SpinLock addCallStatSpinLock = new SpinLock();
        ...
        public static IEnumerable<string> ActivationKeywords => HierarchialConfigurationUpdater.ActivationKeyWords;
        ...
        public void AddCallStat(CallStat lastCallStat)
        {
            ...
            if (logStats != null)
            {
                QueueCalculationOnThreadPool(logStats);
            }
        }
            ...
            get
            {
                return settingTranslation ?? (settingTranslation = (hl, setting) =>
                {
                    ...
                    if (HierarchialConfigurationUpdater.IsActivationKeyWord(setting))
                    {
                        if (csl.BatchSize < 2)
                        {
                            csl.BatchSize = DefaultReportAfter;
                        }
                        return;
                    }
        ...
        {
            return GetCallStatsLogger(name, callingClass.FullName);
        }

        public static CallStatsLogger GetCallStatsLogger(string name, string prefixName)
        {
            var key = GenKey(prefixName, name);
            if (InstanceTracker.TryGetValue(key, out var getInstance))
            {
                return getInstance;
            }
            lock (StaticSyncCntx)
            {
                if (InstanceTracker.TryGetValue(key, out getInstance))
                {
                    return getInstance;
                }
        ...
        private static string GenKey(string prefixName, string name)
        {
        ...
        public void CalculateStats(List<CallStat> callStats)
        {
            ...
            try
            {
                ...
                CallStatsForPercentile quickest50ThPercentile = callStats.QuickestPercentile(0.5);
                CallStatsForPercentile quickest90ThPercentile = callStats.QuickestPercentile(0.9);
                CallStatsForPercentile quickest99ThPercentile = callStats.QuickestPercentile(0.99);
                CallStatsForPercentile allCallStats = callStats.QuickestPercentile(1);
                CallStatsForPercentile slowest5ThPercentile = callStats.SlowestPercentile(0.05);
