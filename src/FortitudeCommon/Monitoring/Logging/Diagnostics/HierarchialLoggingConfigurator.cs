...
{
    public class HierarchialLoggingConfigurator<T> where T : class, IHierarchialLogger
    {
        private static SortedList<string, List<T>> trackingItems;

        static HierarchialLoggingConfigurator()
        {
            if (trackingItems == null)
            {
                trackingItems = new SortedList<string, List<T>>(new LengthLexComparer());
            }
            if (loggingActivations == null)
            {
                loggingActivations = new SortedList<string, string>(new LengthLexComparer());
            }
        }

        public static IEnumerable<string> ActivationKeywords => cachedActivationWords ??
                  (cachedActivationWords = CallStatsLogger.ActivationKeywords.Concat(LatencyLogger.ActivationKeywords)
        ...
        public static void AddRule(string ruleHierarchy, string value)
        {
            ...
            foreach (var item in itemsMatchingRuleName)
            {
                ProcessRulesForLogger(item);
            }
        }
        ...
        private static void ProcessRulesForLogger(T logger)
        {
            ...
            lock (ConfigSyncLock)
            {
                ...
                {
                    if (lowerCaseLoggerFullName.StartsWith(rule.Key))
                    {
                        currentApplied = rule.Value;
                    }
                    if (lowerCaseLoggerFullName.Length < rule.Key.Length)
                    {
                        break;
                    }
                }
        ...
        public static bool AggregateLoggerShouldActivate(string typeNameSpace)
        {
            ...
            lock (ConfigSyncLock)
            {
                if (loggingActivations.ContainsKey(typeNameSpace))
                {
                    setting = loggingActivations[typeNameSpace];
                }
            }
            if (setting == null) return false;
            var settings = setting.Split('-');
            if (settings.Any(aSettingValue => ActivationKeywords.Contains(aSettingValue.ToLower())))
            {
                return true;
            }
        ...
        private static SortedList<string, string> loggingActivations;
        private static readonly object TrackingSyncLock = new object();
        private static readonly object ConfigSyncLock = new object();
