...
namespace FortitudeCommon.Extensions
{
    ...
    {
        public static DiffResults<T> Diff<T, U>(this IEnumerable<T> originalSet, IEnumerable<T> newSet, Func<T, U> uniqueId)
        {
            ...
            foreach (var checkItem in newSet)
            {
                ...
                else
                {
                    ...
                    originalSetDict.Remove(checkItemKey);
                    if (!originalItem.Equals(checkItem))
                    {
                        updatedItems.Add(checkItem);
                    }
                }
            }
