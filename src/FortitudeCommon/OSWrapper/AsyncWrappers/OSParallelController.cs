...
namespace FortitudeCommon.OSWrapper.AsyncWrappers
{
    ...
    {
        private AutoResetEvent neverTriggedResetEvent;

        private AutoResetEvent NeverTriggered
            => neverTriggedResetEvent ?? (neverTriggedResetEvent = new AutoResetEvent(false));

        public IOSThread CreateNewOSThread(ThreadStart threadStart)
        {
            return new OSThread(new Thread(threadStart));
        }

        public IOSThread CreateNewOSThread(ParameterizedThreadStart threadStart)
        {
            return new OSThread(new Thread(threadStart));
        }
        ...
        {
            return new OSThread(new Thread(threadStart, maxThreadSize));
        }
        ...
        {
            return new IntraOSThreadSignal(new AutoResetEvent(initialeState));
        }

        public IIntraOSThreadSignal AllWaitingOSThreadActivateSignal(bool initialeState)
        {
            return new IntraOSThreadSignal(new ManualResetEvent(initialeState));
        }
