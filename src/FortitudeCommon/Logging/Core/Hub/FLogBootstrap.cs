...
public class FLogBootstrap
{
    ...
    public FLogContext DefaultInitializeContext(FLogContext toInitialize)
    {
        ...
        if (File.Exists(defaultMachineConfigFilePath)) Console.Out.WriteLine("Found FLog config " + defaultMachineConfigFilePath);
        ...
        var fLogAppConfig
            = LoadMandatoryFileConfig(null, [flogResolvedConfigFilePath], defaultMachineConfigFilePath, defaultUserMachineConfigFilePath);
    ...
    private FLogAppConfig LoadMandatoryFileConfig(string? workingDirPath, string[] mustFindOneOfFiles, params string[] allOptionalOverrideFiles)
    {
        ...
        {
            foreach (var mandatoryFile in existsMandatory.Concat(existsOptional)) Console.Out.WriteLine("Found FLog config " + mandatoryFile);

            IConfigurationBuilder configBuilder = new ConfigurationBuilder();
            if (workingDirPath != null) configBuilder.SetBasePath(workingDirPath);
    ...
    public virtual FLogContext InitializeContextFromConfig(FLogContext toInitialize, IFLogAppConfig config) =>
        Initialize((IMutableFLogAppConfig)config, toInitialize);
...
public static class FLogBootstrapExtensions
{
    ...
    {
        if (toInitialize.IsInitialized) throw new InvalidOperationException("Attempting to initialize an already initialized context");
    ...
    public static IFLogContext InitializeContextFromFiles(this IFLogContext toInitialize, string filePath, params string[] optionalOverrideFiles) =>
        FLogBootstrap.Instance.InitializeContextFromFiles((FLogContext)toInitialize, [filePath], optionalOverrideFiles);
    ...
    public static IFLogContext InitializeContextFromWorkingDirFilePath(this IFLogContext toInitialize, string workingDirPath, string filePath) =>
        FLogBootstrap.Instance.InitializeContextFromWorkingDirFilePath((FLogContext)toInitialize, workingDirPath, filePath);

    public static IFLogContext StartFlogSetAsCurrentContext(this IFLogContext toStart)
    {
        if (!toStart.IsInitialized) throw new InvalidOperationException("Attempting to start an uninitialized FLog context");
    ...
    {
        if (!toInitializeAndStart.IsInitialized) toInitializeAndStart.DefaultInitializeContext();
        if (!toInitializeAndStart.HasStarted) toInitializeAndStart.StartFlogSetAsCurrentContext();
    ...
    {
        if (toInitializeAndStart.IsInitialized) throw new InvalidOperationException("Attempting to initialize an already initialized context");
    ...
    public static IFLogContext InitializeContextFromFiles(this IFLogContext toInitialize, string[] atLeastOneMandatoryConfigFiles
      , params string[] optionalOverrideFiles) =>
        FLogBootstrap.Instance.InitializeContextFromFiles
