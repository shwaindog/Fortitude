...
public abstract partial class FLogEntryMessageBuilder
{
    ...
    {
        for (var i = 0; i < SupportedRangeIndexTypes.Length; i++)
            if (SupportedRangeIndexTypes[i] == checkIsSupported)
                return true;
        return false;
    }
    ...
    private Action<T, ITheOneString>? CheckSingleFieldFor2ItemTupleInvoker<T>(T tuple, Type tupleType, Type item1Type, Type item2Type)
    {
        ...
        if (item2Type.IsGenericType)
        {
            ...
            if (item1Type.IsValueType && item2GenericType == typeof(PalantírReveal<>))
            {
      ...
      , Type item2Type)
    {
        ...
        for (var i = 0; i < MyNonPubStaticMethods.Length; i++)
        {
            ...
            if (p1Item1IsNullable) p1IsSpanFormattable = firstParamType.GenericTypeArguments[0].IsSpanFormattable();
