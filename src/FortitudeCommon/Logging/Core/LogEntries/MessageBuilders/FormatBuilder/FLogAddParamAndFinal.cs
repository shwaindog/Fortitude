...
public partial class FLogAdditionalFormatterParameterEntry
{
    ...
    public void AndFinalParam<TToStyle, TStylerType>(TToStyle value, PalantírReveal<TStylerType> palantírReveal) where TToStyle : TStylerType =>
        PreCheckTokensGetStringBuilder(value).ReplaceCustStyleTokens(value, palantírReveal).CallEnsureNoMoreTokensAndComplete(value);

    public void AndFinalParam<TToStyle, TStylerType>((TToStyle, PalantírReveal<TStylerType>) valueTuple) where TToStyle : TStylerType
    {
        FormatSb.Clear();
