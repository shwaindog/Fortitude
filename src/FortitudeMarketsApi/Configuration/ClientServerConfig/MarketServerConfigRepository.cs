...
{
    public class MarketServerConfigRepository<T> : IMarketServerConfigRepository<T>, IMuteableRepository<T> where T : class, IMarketServerConfig<T>
    {
        public MarketServerConfigRepository()
        {
            currentConfigs = new Dictionary<long, T>();
        }
        ...
        public IObservable<IMarketServerConfigUpdate<T>> ServerConfigUpdateStream
        {
            get
            {
                ...
                {
                    foreach (T currentServerConfig in CurrentConfigs)
                    {
        ...
        {
            return currentConfigs.Select(kvp => kvp.Value).FirstOrDefault(sci => sci.Name.Equals(name, StringComparison.InvariantCultureIgnoreCase));
        }

        public bool ContainsItem(T checkItem)
        {
            return currentConfigs.ContainsKey(checkItem.Id);
        }
